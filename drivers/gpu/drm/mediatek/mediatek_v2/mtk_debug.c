...
static int mtk_dp_modes_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;

	return 0;
}
static ssize_t mtk_dp_modes_read(struct file *file, char __user *ubuf, size_t count,
			  loff_t *ppos)
{
	char buf[128];
	int cur_hdisplay, cur_vdisplay, cur_vrefresh;
	int prefer_hdisplay, prefer_vdisplay, prefer_vrefresh;
	int cnt = 0;

	memset(buf, 0, 128);
	mtk_dp_get_switched_modes(&cur_hdisplay, &cur_vdisplay, &cur_vrefresh,
					&prefer_hdisplay, &prefer_vdisplay, &prefer_vrefresh);
	sprintf(buf, "cur:%d-%d-%d-%d prefer:%d-%d-%d-%d", cur_hdisplay, cur_vdisplay, cur_vrefresh, 0,
					prefer_hdisplay, prefer_vdisplay, prefer_vrefresh, 0);
	cnt = strlen(buf) + 1;

	return simple_read_from_buffer(ubuf, count, ppos, buf, cnt);
}

static ssize_t mtk_dp_modes_write(struct file *file, const char __user *ubuf,
			   size_t count, loff_t *ppos)
{
	char buf[64];
	...
	mtk_dp_switch_ext_modes(hdisplay, vdisplay, vrefresh, aspect_ratio);
clear:
	return count;
}

static const struct proc_ops mtk_dp_modes_proc_fops = {
	.proc_read = mtk_dp_modes_read,
	.proc_write = mtk_dp_modes_write,
	.proc_open = mtk_dp_modes_open,
};
...
void disp_dbg_probe(void)
{
	...
	if (!proc_create("dp_modes", S_IFREG | 0444,
		mtkfb_debug_procfs, &mtk_dp_modes_proc_fops)) {
		DDPPR_ERR("[%s %d]failed to create idlevfp in /proc/mtkfb_debug/dp_modes\n",
			__func__, __LINE__);
		goto out;
	}
