...
void disp_pq_notify_backlight_changed(struct mtk_ddp_comp *comp, int bl_1024)
{
	...
	if (pq_data->new_persist_property[DISP_PQ_CCORR_SILKY_BRIGHTNESS]) {
      ...
      if ((primary_data->old_pq_backlight == 0) && (primary_data->pq_backlight != 0))
		disp_ccorr_refresh_by_kernel(comp);
}
...
static int mtk_disp_ccorr_set_interrupt(struct mtk_ddp_comp *comp, void *data)
{
	...
	int ret = 0;
        DDPINFO("%s: jie debug: set ccorr Interrupt: %d\n", __func__, enabled);
	if (enabled) {
		if (ccorr_data->path_order == 0)
...
void disp_ccorr_refresh_by_kernel(struct mtk_ddp_comp *comp)
{
	unsigned long clockflags;
	int enabled = 1;
	struct mtk_disp_ccorr *ccorr_data = comp_to_ccorr(comp);
	struct mtk_disp_ccorr_primary *primary_data = ccorr_data->primary_data;
		...
		spin_lock_irqsave(&primary_data->ccorr_clock_lock, clockflags);
		if (atomic_read(&ccorr_data->is_clock_on) != 1)
			DDPINFO("ccorr clock is off\n");
		else if (comp->mtk_crtc->is_dual_pipe && atomic_read(&ccorr_data->is_clock_on) != 1)
			DDPINFO("ccorr1 clock is off\n");
		else
			mtk_disp_ccorr_set_interrupt(comp, &enabled);

		spin_unlock_irqrestore(&primary_data->ccorr_clock_lock, clockflags);

		/* Backlight or Kernel API latency should be smallest */
		/* for video mode panel, panel will refresh fram */
