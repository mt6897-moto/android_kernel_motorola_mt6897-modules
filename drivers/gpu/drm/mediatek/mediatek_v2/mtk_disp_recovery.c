...
bool lcd_esd_check_flag = 0;
EXPORT_SYMBOL_GPL(lcd_esd_check_flag);
//-checlist-98559,fangzhihua.wt,add,20240328,lcd esd recovery notifier suspend/resume to TP
...
int mtk_drm_esd_testing_process(struct mtk_drm_esd_ctx *esd_ctx, bool need_lock)
{
		...
		do {
			...
			lcd_esd_check_flag = 1;//checlist-98559,fangzhihua.wt,add,20240328,lcd esd recovery notifier suspend/resume to TP
		...
		if (ret != 0) {
			...
			recovery_flg = 0;
			lcd_esd_check_flag = 0;//checlist-98559,fangzhihua.wt,add,20240328,lcd esd recovery notifier suspend/resume to TP
		}
