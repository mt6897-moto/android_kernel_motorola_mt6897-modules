...
void mdrv_DPTx_SetDPTXOut(struct mtk_dp *mtk_dp)
{
	...
	DPTXMSG("mdrv_DPTx_SetDPTXOut is %d\n", mtk_dp->info.input_src);
...
void mdrv_DPTx_Video_Enable(struct mtk_dp *mtk_dp, bool bEnable)
{
	...
	val = msRead4Byte(mtk_dp, 0x113c);
	DPTXMSG("0x113c 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1138);
	DPTXMSG("0x1138 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1140);
	DPTXMSG("0x1140 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1144);
	DPTXMSG("0x1144 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1148);
	DPTXMSG("0x1148 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x114c);
	DPTXMSG("0x114c 0x%x\n", val);

	val = msRead4Byte(mtk_dp, 0x1238);
	DPTXMSG("0x1238 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x123c);
	DPTXMSG("0x123c 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1240);
	DPTXMSG("0x1240 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1244);
	DPTXMSG("0x1244 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1248);
	DPTXMSG("0x1248 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x124c);
	DPTXMSG("0x124c 0x%x\n", val);

	val = msRead4Byte(mtk_dp, 0x1338);
	DPTXMSG("0x1338 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x133c);
	DPTXMSG("0x133c 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1340);
	DPTXMSG("0x1340 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1344);
	DPTXMSG("0x1344 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1348);
	DPTXMSG("0x1348 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x134c);
	DPTXMSG("0x134c 0x%x\n", val);

	val = msRead4Byte(mtk_dp, 0x1438);
	DPTXMSG("0x1438 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x143c);
	DPTXMSG("0x143c 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1440);
	DPTXMSG("0x1440 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1444);
	DPTXMSG("0x1444 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x1448);
	DPTXMSG("0x1448 0x%x\n", val);
	val = msRead4Byte(mtk_dp, 0x144c);
	DPTXMSG("0x144c 0x%x\n", val);
...
static int mtk_dp_conn_get_modes(struct drm_connector *conn)
{
	...
	if (fakecablein) {
		...
		count = dp_modes_probing_get_modes_count();
		target_mode_index = dp_modes_probing_get_selected_index();
		if(0>=count || FAKE_DEFAULT_RES==target_mode_index) 
			return 0;

		for(i=0; i<count; i++) {
			ptmp = dp_modes_probing_get(i);
			if(!ptmp) break;
			mode = drm_mode_duplicate(dev, (struct drm_display_mode*)ptmp);
			drm_mode_set_name(mode);
			mode->type = DRM_MODE_TYPE_DRIVER;
			printk("check i = %d ,count = %d, target_mode_index = %d\n",i,count,target_mode_index);
			if(i == target_mode_index) {
				mode->type |= DRM_MODE_TYPE_PREFERRED;
				DPTXFUNC("set preferred! the index is %d\n", i);
			} else {
				mode->type &= ~DRM_MODE_TYPE_PREFERRED;
			}
	...
	if (mtk_dp->edid) {
		...
		DPTXMSG("%s modes = %d\n", __func__, ret);
		list_for_each_entry_safe(pmode, pt, &mtk_dp->conn.probed_modes, head) {
			DPTXDBG("modes: %d*%d@%d clk\n", pmode->hdisplay, pmode->vdisplay, pmode->clock);
		}
...
//+add del low resolution
        if ((mode->vdisplay < 900) || (mode->hdisplay < 900))
                return MODE_CLOCK_HIGH;

        if ((mode->vdisplay > 4000) || (mode->hdisplay > 4000)) {
                return MODE_CLOCK_HIGH;
        } /*else if ((mode->vdisplay > 2000) || (mode->hdisplay > 2000)) {
...
*/
	...
	DPTXMSG("%s xres=%d, yres=%d, refresh=%d, clock=%d\n",
			...
			mode->clock);
	DPTXMSG("%s htt=%d, hse=%d, hss=%d, vse=%d, vst=%d, vtt=%d\n",
			__func__, mode->htotal, mode->hsync_end,mode->hsync_start,
			mode->vsync_end, mode->vsync_start, mode->vtotal);
	DPTXMSG("%s hskew=%d, vscan=%d, flags=%d, type=%d\n",
			__func__, mode->hskew, mode->vscan, mode->flags, mode->type);
	DPTXMSG("%s picture_aspect_ratio=%d, crtc_vtotal=%d, crtc_htotal=%d, status=%d\n",
			__func__, mode->picture_aspect_ratio, mode->crtc_vtotal, mode->crtc_htotal, mode->status);
	DPTXMSG("mode END\n");
	...
	if(!fakecablein) {
		dp_modes_probing_add((void*)mode);
		if(mode->type & DRM_MODE_TYPE_PREFERRED) {
			dp_prefer_mode.hdisplay = mode->hdisplay;
			dp_prefer_mode.vdisplay = mode->vdisplay;
			dp_prefer_mode.fps = drm_mode_vrefresh(mode);
		}
	}
...
void mtk_dp_hotplug_uevent(unsigned int event)
{
	...
	if(1==event && !fakecablein) {
		dp_modes_probing_clear();
		dp_prefer_mode.hdisplay = 0;
		dp_prefer_mode.vdisplay = 0;
		dp_prefer_mode.fps = 0;
	}
...
void mtk_dp_fake_plugin(unsigned int status, unsigned int bpc)
{
	...
	DPTXFUNC("fakecablein is %d\n", fakecablein);
	g_mtk_dp->state = DPTXSTATE_INITIAL;
//	g_mtk_dp->training_state = DPTX_NTSTATE_STARTUP;
//	queue_work(g_mtk_dp->dptx_wq, &g_mtk_dp->dptx_work);
	mtk_dp_hotplug_uevent(1);
}
...
void mtk_dp_HPDInterruptSet(int bstatus)
{
	...
	if (bstatus == HPD_INT_EVNET && !g_mtk_dp->dp_ready)
		return;
