...
struct mtk_dp_ext_modes dptx_curr_mode;
struct mtk_dp_ext_modes dp_prefer_mode;

static const struct mtk_dp_ext_modes dp_ext_modes[] = {
	{3840, 2160, 60, SINK_3840_2160},
	{3840, 2160, 30, SINK_3840_2160_30},
	{1920, 1080, 60, SINK_1920_1080},
	{1280, 720, 60, SINK_1280_720},
	{640, 480, 60, SINK_640_480},
};
...
void mtk_dp_switch_ext_modes(int h, int v, int fps, int aspect_ratio)
{
	...
	if (h ==dptx_curr_mode.hdisplay && v==dptx_curr_mode.vdisplay
		&& fps == dptx_curr_mode.fps) {
		DPTXMSG("same modes.\n");
		return;
	}

	status = dp_modes_probing_select(h, v, fps);
	DPTXMSG("dp_modes_select, status=%ud.\n", status);
	if(0xFF!=status) {
		mtk_dp_fake_plugin(status, 1);
	}
}

void mtk_dp_get_switched_modes(int *cur_h, int *cur_v, int *cur_fps,
				int *prefer_h, int *prefer_v, int *prefer_fps) {
	*cur_h = dptx_curr_mode.hdisplay;
	*cur_v = dptx_curr_mode.vdisplay;
	*cur_fps = dptx_curr_mode.fps;
	*prefer_h = dp_prefer_mode.hdisplay;
	*prefer_v = dp_prefer_mode.vdisplay;
	*prefer_fps = dp_prefer_mode.fps;
