...
//+Peridot-35, liyiying.wt, 20240612, add, add del usb redriver
static bool ps5170_device_exits;
static int device_exits(void)
{
	...
	if (psy == NULL) {
		psy = power_supply_get_by_name("PS5170");
	}
	...
	if (psy == NULL || IS_ERR(psy)) {
		...
		is_exits = val.intval;
		pr_err("%s Can get chg_psy, val is %d %d\n", __func__, is_exits, val.intval);
	}

	if (is_exits == 0 || is_exits == 2) {
		return false;
	} else if (is_exits == 1){
		ps5170_device_exits = true;
		return true;
	} else {
		return false;
	}
...
static bool is_exit_ps1570(void)
{

	if(device_exits() == true || ps5170_device_exits == true) {
		DPTXMSG("%s ps5170 is exits\n", __func__);
		return true;
	} else {
		DPTXMSG("%s ps5170 is not exits\n", __func__);
		return false;
	}
}

static unsigned int values[24];
static bool run_one_times = false;
static void save_ori_state(struct mtk_dp *mtk_dp)
{
	int i = 0;

	values[0] = msRead4Byte(mtk_dp, 0x1138);
	values[1] = msRead4Byte(mtk_dp, 0x113c);
	values[2] = msRead4Byte(mtk_dp, 0x1140);
	values[3] = msRead4Byte(mtk_dp, 0x1144);
	values[4] = msRead4Byte(mtk_dp, 0x1148);
	values[5] = msRead4Byte(mtk_dp, 0x114c);

	values[6] = msRead4Byte(mtk_dp, 0x1238);
	values[7] = msRead4Byte(mtk_dp, 0x123c);
	values[8] = msRead4Byte(mtk_dp, 0x1240);
	values[9] = msRead4Byte(mtk_dp, 0x1244);
	values[10] = msRead4Byte(mtk_dp, 0x1248);
	values[11] = msRead4Byte(mtk_dp, 0x124c);

	values[12] = msRead4Byte(mtk_dp, 0x1338);
	values[13] = msRead4Byte(mtk_dp, 0x133c);
	values[14] = msRead4Byte(mtk_dp, 0x1340);
	values[15] = msRead4Byte(mtk_dp, 0x1344);
	values[16] = msRead4Byte(mtk_dp, 0x1348);
	values[17] = msRead4Byte(mtk_dp, 0x134c);

	values[18] = msRead4Byte(mtk_dp, 0x1438);
	values[19] = msRead4Byte(mtk_dp, 0x143c);
	values[20] = msRead4Byte(mtk_dp, 0x1440);
	values[21] = msRead4Byte(mtk_dp, 0x1444);
	values[22] = msRead4Byte(mtk_dp, 0x1448);
	values[23] = msRead4Byte(mtk_dp, 0x144c);
...
void mhal_DPTx_PHYSetting(struct mtk_dp *mtk_dp)
{
	uint32_t value = 0;
	uint8_t mask = 0x3F;
	static bool is_ps5170_no_exit = false;
...
//PORTING FROM CTP END
	...
	//+Peridot-35, liyiying.wt, 20240612, add, add del usb redriver
	if (run_one_times == false) {
		save_ori_state(mtk_dp);
		run_one_times = true;
	}

	if (is_exit_ps1570() == false) {
		is_ps5170_no_exit = true;

		msWrite4Byte(mtk_dp, 0x1138, 0x20161410);
		msWrite4Byte(mtk_dp, 0x113c, 0x26221c18);
		msWrite4Byte(mtk_dp, 0x1140, 0x3028);
		msWrite4Byte(mtk_dp, 0x1144, 0x10060400);
		msWrite4Byte(mtk_dp, 0x1148, 0x80E0400);
		msWrite4Byte(mtk_dp, 0x114c, 0x8);

		msWrite4Byte(mtk_dp, 0x1238, 0x20161410);
		msWrite4Byte(mtk_dp, 0x123c, 0x26221c18);
		msWrite4Byte(mtk_dp, 0x1240, 0x3028);
		msWrite4Byte(mtk_dp, 0x1244, 0x10060400);
		msWrite4Byte(mtk_dp, 0x1248, 0x80E0400);
		msWrite4Byte(mtk_dp, 0x124c, 0x8);

		msWrite4Byte(mtk_dp, 0x1338, 0x20161410);
		msWrite4Byte(mtk_dp, 0x133c, 0x26221c18);
		msWrite4Byte(mtk_dp, 0x1340, 0x3028);
		msWrite4Byte(mtk_dp, 0x1344, 0x10060400);
		msWrite4Byte(mtk_dp, 0x1348, 0x80E0400);
		msWrite4Byte(mtk_dp, 0x134c, 0x8);

		msWrite4Byte(mtk_dp, 0x1438, 0x20161410);
		msWrite4Byte(mtk_dp, 0x143c, 0x26221c18);
		msWrite4Byte(mtk_dp, 0x1440, 0x3028);
		msWrite4Byte(mtk_dp, 0x1444, 0x10060400);
		msWrite4Byte(mtk_dp, 0x1448, 0x80E0400);
		msWrite4Byte(mtk_dp, 0x144c, 0x8);
	}
	else if (is_exit_ps1570() == true && is_ps5170_no_exit == true) {
		is_ps5170_no_exit = false;
		msWrite4Byte(mtk_dp, 0x1138, values[0]);
		msWrite4Byte(mtk_dp, 0x113c, values[1]);
		msWrite4Byte(mtk_dp, 0x1140, values[2]);
		msWrite4Byte(mtk_dp, 0x1144, values[3]);
		msWrite4Byte(mtk_dp, 0x1148, values[4]);
		msWrite4Byte(mtk_dp, 0x114c, values[5]);

		msWrite4Byte(mtk_dp, 0x1238, values[6]);
		msWrite4Byte(mtk_dp, 0x123c, values[7]);
		msWrite4Byte(mtk_dp, 0x1240, values[8]);
		msWrite4Byte(mtk_dp, 0x1244, values[9]);
		msWrite4Byte(mtk_dp, 0x1248, values[10]);
		msWrite4Byte(mtk_dp, 0x124c, values[11]);

		msWrite4Byte(mtk_dp, 0x1338, values[12]);
		msWrite4Byte(mtk_dp, 0x133c, values[13]);
		msWrite4Byte(mtk_dp, 0x1340, values[14]);
		msWrite4Byte(mtk_dp, 0x1344, values[15]);
		msWrite4Byte(mtk_dp, 0x1348, values[16]);
		msWrite4Byte(mtk_dp, 0x134c, values[17]);

		msWrite4Byte(mtk_dp, 0x1438, values[18]);
		msWrite4Byte(mtk_dp, 0x143c, values[19]);
		msWrite4Byte(mtk_dp, 0x1440, values[20]);
		msWrite4Byte(mtk_dp, 0x1444, values[21]);
		msWrite4Byte(mtk_dp, 0x1448, values[22]);
		msWrite4Byte(mtk_dp, 0x144c, values[23]);
	}
	//-Peridot-35, liyiying.wt, 20240612, add, add del usb redriver
}
