...
static void mtk_crtc_update_hrt_usage(struct drm_crtc *crtc, bool hrt_valid)
{
	...
	if (mtk_crtc->is_dual_pipe) {
		drm_for_each_plane_mask(plane, crtc->dev, plane_mask) {
			...
			struct mtk_ddp_comp *comp = mtk_crtc_get_plane_comp(crtc, plane_state);
			unsigned int comp_id = dual_pipe_comp_mapping(mtk_get_mmsys_id(crtc), comp->id);
			struct mtk_ddp_comp *mapping_comp = priv->ddp_comp[comp_id];

			mtk_ddp_comp_io_cmd(mapping_comp, NULL, OVL_PHY_USAGE, plane_state);
		}
	}
#endif
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	if ((index == 0 || index == 3 || mtk_crtc->path_data->is_discrete_path) && hrt_valid == false) {
			...
			unsigned int crtc_id)
{
	...
	/* this crtc OVL usage non-conflict with main display, enable */
	if ((priv->ovl_usage[main_disp_idx] & priv->ovl_usage[crtc_id]) == 0) {
		DDPMSG("%s: crtc%d non conflict with main display, enable directly\n", __func__, crtc_id);
		priv->usage[crtc_id] = DISP_ENABLE;
		mutex_unlock(&priv->res_usage_lock);
		return DISP_ENABLE;
	}
...
void mtk_set_dpc_dsi_clk(struct mtk_drm_crtc *mtk_crtc, bool enable)
{
...
void mtk_crtc_first_enable_ddp_config(struct mtk_drm_crtc *mtk_crtc)
{
	...
	/*1. Show LK logo only */
    //+PERIDOT-13093, wenhui.wt, add, 20240906, meta mode keep display =>META MODE
	if (META_BOOT != priv->boot_mode)
		mtk_crtc_all_layer_off(mtk_crtc, cmdq_handle);
	DDPMSG("%s: boot_mode = %d\n", __func__, priv->boot_mode);
    //-PERIDOT-13093, wenhui.wt, add, 20240906, meta mode keep display =>META MODE
