...
#include <linux/hardware_info.h> //checklist-98164,zhangshaoxiong1.wt,modify,20240221,add lcd hardware info
	...
	struct mtk_plane_state *mtk_plane_state)
{
	...
	submit_kernel->buffer.src.cnt = max_t(uint8_t, min_t(uint8_t,
						submit_kernel->buffer.src.cnt, MML_MAX_PLANES), 1);
			...
			unsigned int *fps)
{
	...
	char hardinfo[HARDWARE_MAX_ITEM_LONGTH];
	memset(hardinfo,0,HARDWARE_MAX_ITEM_LONGTH);
	//-checklist-98164,zhangshaoxiong1.wt,modify,20240221,add lcd hardware info
	...
	if (chosen_node) {
		...
		if (videolfb_tag) {
			...
			*fps = videolfb_tag->fps;
			//+checklist-98164,zhangshaoxiong1.wt,modify,20240221,add lcd hardware info
			snprintf(hardinfo,HARDWARE_MAX_ITEM_LONGTH,videolfb_tag->lcmname);
			hardwareinfo_set_prop(HARDWARE_LCD,hardinfo);
			//-checklist-98164,zhangshaoxiong1.wt,modify,20240221,add lcd hardware info
			if (*fps == 0)
				*fps = 6000;
			goto found;
