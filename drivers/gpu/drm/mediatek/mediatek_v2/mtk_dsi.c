...
static void mtk_dsi_calc_vdo_timing(struct mtk_dsi *dsi)
{
	...
	/*DDPINFO("[QZ_Debug]%s , dsi->is_slave = %d, dsi->mipi_hopping_sta = %d, \n",
                    __func__, dsi->is_slave, dsi->mipi_hopping_sta);
    DDPINFO("[QZ_Debug]%s , vm: hfront_porch = %d; hback_porch = %d; vfront_porch = %d; \n",
            __func__, vm->hfront_porch, vm->hback_porch, vm->vfront_porch);*/
	...
	t_hsa = (dsi->mipi_hopping_sta) ?
			...
			vm->hsync_len;
	/*DDPINFO("[QZ_Debug]%s , dsi->is_slave = %d,t_hbp = %u, \n",
                    __func__, dsi->is_slave, t_hbp);*/
...
static void mtk_dsi_config_vdo_timing(struct mtk_dsi *dsi)
{
	...
	/*DDPINFO("[QZ_Debug]%s , dsi->is_slave = %d, dsi->hbp_byte = %u \n",
                    __func__, dsi->is_slave, dsi->hbp_byte);*/
	...
	int force_lcm_update)
{
    ...
    //+Peridot-32,zhangshaoxiong1.wt,modify,20240425,power on vddi pin before mipi output
    if (dsi->ext && dsi->ext->funcs &&
            dsi->ext->funcs->power_on_pin){
        dsi->ext->funcs->power_on_pin(dsi->panel);
        DDPINFO("[zsx]Power on vddi pin before mipi output.");
    }
    mdelay(2);
	//-Peridot-32,zhangshaoxiong1.wt,modify,20240425,power on vddi pin before mipi output
	...
	if (dsi->panel) {
		...
		//printk("%s,chg index:0x%x\n", __func__, mode_chg_index);

		/* add for ESD recovery */
		if ((mtk_crtc->res_switch == RES_SWITCH_NO_USE)
			&& (mode_id != 0)
			&& (mtk_dsi_is_cmd_mode(&dsi->ddp_comp) ||
				(mode_chg_index & MODE_DSI_HFP) || (mode_chg_index & MODE_DSI_VFP))) {  //Peridot-32,zhangshaoxiong1.wt,modify,20240410,add mode_switch condition after esd recovery
			if (dsi->ext && dsi->ext->funcs &&
...
SKIP_WAIT_FRAME_DONE:
	...
	mdelay(2);
    if (dsi->ext && dsi->ext->funcs &&
            dsi->ext->funcs->power_off_pin){
        dsi->ext->funcs->power_off_pin(dsi->panel);
        DDPINFO("[zsx]Power off vddi pin anter mipi down.");
    }
    mdelay(80);
	//-Peridot-32,zhangshaoxiong1.wt,modify,20240425,power on vddi pin before mipi output
	DDPINFO("%s-\n", __func__);
			  ...
			  enum dsi_porch_type type, unsigned int value)
{
	...
	case DSI_HBP:
		...
		/*DDPINFO("[QZ_Debug]%s , dsi->is_slave = %d, DSI_HBP_WC = %d \n",
                    __func__, dsi->is_slave, value);*/
		break;
	case DSI_BLLP:
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (fps_chg_index & MODE_DSI_CLK) {
		...
		/*DDPINFO("[QZ_Debug]%s + , slave_dsi->vm: hfront_porch = %d; hback_porch = %d; vfront_porch = %d; \n",
                    __func__, dsi->slave_dsi->vm.hfront_porch, dsi->slave_dsi->vm.hback_porch, dsi->slave_dsi->vm.vfront_porch);*/
		//+Peridot-32,zhangshaoxiong1.wt,modify,20240321,update dsi1 porch after esd recovery
        if (dsi->slave_dsi) {
            dsi->slave_dsi->vm.hfront_porch = hfp;
            dsi->slave_dsi->vm.hback_porch = hbp;
...
int PanelMaster_DSI_set_timing(struct mtk_dsi *dsi, struct MIPI_TIMING timing)
{
	...
	case MIPI_HBP:
	{
		...
		/*DDPINFO("[QZ_Debug]%s , dsi->is_slave = %d, DSI_HBP_WC = %u \n",
                    __func__, dsi->is_slave, hbp_byte);*/
		break;
	}
	case MIPI_VPW:
	{
