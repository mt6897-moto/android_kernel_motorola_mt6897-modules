	...
	const unsigned int hrt_idx)
{
	...
	for (i = 0; i < disp_info->layer_num[disp_idx]; i++) {
		...
		/* Try to rollback to DC if plane not same cross-pipe */
		if (MTK_MML_DISP_DIRECT_DECOUPLE_LAYER & c->layer_caps ||
			MTK_MML_DISP_DIRECT_LINK_LAYER  & c->layer_caps) {
			if ((c->dst_offset_x < 1472) && (c->dst_width + c->dst_offset_x > 1472) &&
				((1472 - c->dst_offset_x) != (c->dst_width + c->dst_offset_x - 1472) )) {
				c->layer_caps &= ~MTK_MML_DISP_DIRECT_DECOUPLE_LAYER;
				c->layer_caps &= ~MTK_MML_DISP_DIRECT_LINK_LAYER;
				c->layer_caps |= MTK_MML_DISP_DECOUPLE_LAYER;
				DDPINFO("%s dst_x:%d dst_w:%d not same dst cross-pipe, set to MML_DC\n",
					__func__, c->dst_offset_x, c->dst_width);
			}
		}
