...
#define FLD_RG_DSI_DEM_EN  (0x1 << 9)
...
static int mtk_mipi_tx_pll_prepare_mt6897(struct clk_hw *hw)
{
	...
	mtk_mipi_tx_set_bits(mipi_tx, MIPITX_PLL_CON1,
			       mipi_tx->driver_data->dsi_pll_en);
//+Peridot-32,zhangshaoxiong1.wt,modify,20240425,increase mipi tx driver capability and open DEM_EN func
#if 1 ///As DEM_EN is only need on a specific project, please add a define to distinguish
    //pr_notice("[QZ+] %s, MIPITX_LANE_CON_MT6983=0x%x\n", __func__, readl(mipi_tx->regs + MIPITX_LANE_CON_MT6983));
    mtk_mipi_tx_update_bits(mipi_tx, MIPITX_LANE_CON_MT6983, FLD_RG_DSI_DEM_EN, 1 << 9);
    //pr_notice("[QZ-] %s, MIPITX_LANE_CON_MT6983=0x%x\n", __func__, readl(mipi_tx->regs + MIPITX_LANE_CON_MT6983));
#endif
//-Peridot-32,zhangshaoxiong1.wt,modify,20240425,increase mipi tx driver capability and open DEM_EN func
