				  ...
				  const char *name)
{
	...
	user_buf->cnt = min_t(uint8_t, user_buf->cnt, MML_MAX_PLANES);
	if (user_buf->use_dma)
	...
	void *cb_param)
{
	...
	if (cfg->info.mode == MML_MODE_RACING) {
		...
		/* also mark begin so that disp clear target line event */
		if (atomic_inc_return(&ctx->racing_cnt) == 1 && submit->disp_vdo)
			ctx->racing_begin = true;
...
int mml_ctx_init(struct mml_drm_ctx *ctx, const char * const threads[])
{
	...
	if (threads[2]) {
		...
		if (IS_ERR_OR_NULL(ctx->kt_config[0])) {
	...
	if (threads[3]) {
		...
		if (IS_ERR_OR_NULL(ctx->kt_config[1])) {
