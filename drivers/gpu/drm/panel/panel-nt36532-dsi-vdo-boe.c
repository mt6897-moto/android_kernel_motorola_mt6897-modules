...
static bool tp_wakeup_flag = false;    
void set_boe_tp_wakeup_flag(bool flag)
{
	tp_wakeup_flag = flag;
}
EXPORT_SYMBOL_GPL(set_boe_tp_wakeup_flag);
//-peridot-179,fangzhihua.wt,add, 20240307, add wakeup by TP double-click
...
static void boe_panel_init(struct boe *ctx)
{
	...
	boe_dcs_write_seq_static(ctx, 0xD0, 0x71);
	boe_dcs_write_seq_static(ctx, 0xD1, 0x84);
	boe_dcs_write_seq_static(ctx, 0xD2, 0x38);
	boe_dcs_write_seq_static(ctx, 0xDE, 0x43);
	boe_dcs_write_seq_static(ctx, 0xDF, 0x02);
	//-Peridot-32,zhangshaoxiong1.wt,add,20240328,add TE detection code
	//+Peridot-32,zhangshaoxiong1.wt,add,20240328,add TE detection code
	boe_dcs_write_seq_static(ctx, 0xFF, 0x20);
	boe_dcs_write_seq_static(ctx, 0xFB, 0x01);
	boe_dcs_write_seq_static(ctx, 0x0D, 0x43);
	boe_dcs_write_seq_static(ctx, 0x69, 0xDD);
	boe_dcs_write_seq_static(ctx, 0xFF, 0x2A);
	boe_dcs_write_seq_static(ctx, 0xB1, 0x00, 0x01);
	//-Peridot-32,zhangshaoxiong1.wt,add,20240328,add TE detection code
	boe_dcs_write_seq_static(ctx, 0xFF, 0x10);
	boe_dcs_write_seq_static(ctx, 0xFB, 0x01);
	boe_dcs_write_seq_static(ctx, 0x35, 0x00); //Peridot-32,zhangshaoxiong1.wt,add,20240328,open TE detection code
	...
	boe_dcs_write_seq_static(ctx, 0xB2, 0x80);//Peridot-442,zhangshaoxiong1.wt,modify,20240305,modify B2 register to 80 for switching table
...
static int boe_disable(struct drm_panel *panel)
{
	...
	ctx->enabled = false;
//+checlist-98559,fangzhihua.wt,add,20240328,lcd esd recovery notifier suspend/resume to TP
	if (lcd_esd_check_flag){
		printk("esd boe_unprepare");
...
static int boe_unprepare(struct drm_panel *panel)
{
	...
	pr_info("%s debug+++set bl 0\n", __func__);
	_backlight_i2c_write_bytes(BACKLIGHT_ENABLE, 0x1F);
	_backlight_i2c_write_bytes(BRIGHTNESS_CONTOL, 0x75);
	_backlight_i2c_write_bytes(BRIGHTNESS_LSB, 0x00);
	_backlight_i2c_write_bytes(BRIGHTNESS_MSB, 0x00);
	...
	secong_backlight_i2c_write_bytes(BRIGHTNESS_LSB, 0x00);
	secong_backlight_i2c_write_bytes(BRIGHTNESS_MSB, 0x00);
	secong_backlight_i2c_write_bytes(LED_CURRENT, 0x00);

//+peridot-179,fangzhihua.wt,mod, 20240307, add wakeup by TP double-click
	pr_info("%s--tp_wakeup_flag boe:%d\n", __func__,tp_wakeup_flag);
	if (!tp_wakeup_flag){//{  //fangzhihua.wt
		//+Peridot-checklist-98167,zhangshaoxiong1.wt,modify,20240305,modify power on sequence
        ctx->reset_gpio = devm_gpiod_get(ctx->dev, "reset", GPIOD_OUT_HIGH);
        gpiod_set_value(ctx->reset_gpio, 0);
...
//-Peridot-32,zhangshaoxiong1.wt,modify,20240516,modify power on&off sequence
...
static const struct of_device_id boe_of_match[] = {
	{
		.compatible = "boe,nt36532,dsi,vdo",
	},
	{}
};
...
static struct mipi_dsi_driver boe_driver = {
	...
	.remove = boe_remove,
	.driver = {
			.name = "panel-boe-nt36532-dsi-vdo",
			.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("boe wqxga2944 Panel Driver");
MODULE_LICENSE("GPL");
