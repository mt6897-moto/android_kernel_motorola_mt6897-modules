...
#define GOODIX_FINGERPRINT 		"mediatek,goodix-fp"
#define GF_POWER_GPIO  0
#define GF_POWER_EXT_LDO  0
...
static void gf_spi_remove(struct spi_device  *pdev);
...
static void gf_spi_remove(struct spi_device *spi)
{	
...
int gf_parse_dts(struct gf_device *gf_dev)
{
	...
	np = of_find_compatible_node(NULL, NULL, GOODIX_FINGERPRINT);
...
/*static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, GOODIX_FINGERPRINT);
	if (node) {
...
static void gf_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
	...
	/* TODO:  set power  according to actual situation  */
		...
		if(NULL != gf_dev->vdd) {
			...
			if(rc<0) {
				pr_err("---- unable to regulator_set_load 0 ----\n");
			}
			if(regulator_is_enabled(gf_dev->vdd)) {
				regulator_disable(gf_dev->vdd);
				pr_info("---- regulator_disable  ----\n");
			}
		}
#endif
        if (rc) {
...
static int gf_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	gf_debug(INFO_LOG, "[%s] : enter 0428, event = %lu, blank=0x%x\n", __func__, event, blank);
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_ENABLE_SPI_CLK:
        ...
        __pm_wakeup_event(&gf_dev->fp_wakesrc, 500);
...
static int gf_platform_probe(struct platform_device *pldev)
{
	...
	gf_debug(INFO_LOG, "%s, Setting gf device configuration 0428 ==========\n", __func__);
