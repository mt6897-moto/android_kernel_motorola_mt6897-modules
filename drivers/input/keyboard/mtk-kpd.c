...
#define COM_KEY_TIMEOUT 8
static atomic_t kp_wakeup_flag = ATOMIC_INIT(1);

//+PERIDOT-359,zhaizhenhong.wt,add,20240313,add force shutdown by pressing voldn+powerkey for 8s	
struct work_struct force_shutdown_w;
EXPORT_SYMBOL(force_shutdown_w);

int pwrkey_dn, voldn_dn;
EXPORT_SYMBOL(pwrkey_dn);
EXPORT_SYMBOL(voldn_dn);

struct timer_list com_key_timer;
EXPORT_SYMBOL(com_key_timer);
//-PERIDOT-359,zhaizhenhong.wt,add,20240313,add force shutdown by pressing voldn+powerkey for 8s	
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16U; j++) {
			...
			pr_notice("report Linux keycode = %d\n", keycode);
    			//+PERIDOT-359,zhaizhenhong.wt,add,20240313,add force shutdown by pressing voldn+powerkey for 8s	
			if (keycode == 114) {//KEY_VOLUMEDOWN
				voldn_dn = !!pressed;
				pr_debug("pwrkey:%d vol_dn:%d!\n",pwrkey_dn,voldn_dn);
				if (pwrkey_dn && voldn_dn) {
					com_key_timer.expires = jiffies + msecs_to_jiffies(COM_KEY_TIMEOUT*1000);
					if (!timer_pending(&com_key_timer)){
						add_timer(&com_key_timer);
						pr_debug("ADD TIMER: force shutdown!\n");
					}
				} else if (!voldn_dn) {
					if (timer_pending(&com_key_timer)){
						del_timer_sync(&com_key_timer);
						pr_debug("cancel force shutdown timer!\n");
					}
				}
			}
			//-PERIDOT-359,zhaizhenhong.wt,add,20240313,add force shutdown by pressing voldn+powerkey for 8s	
...
//+PERIDOT-359,zhaizhenhong.wt,add,20240313,add force shutdown by pressing voldn+powerkey for 8s	
static void kpd_force_shutdown_workfunc(struct work_struct *work)
{
	pr_err("[%s] force shutdown by long press voldn + pwrkey 8s!\n", __func__);
	kernel_power_off();
}

void force_shutdown_timeout(struct timer_list *t){
	int ret;
	ret = schedule_work(&force_shutdown_w);
	pr_notice("handle timer of com_key,schedule :%d\n",ret);
}
EXPORT_SYMBOL(force_shutdown_timeout);
//-PERIDOT-359,zhaizhenhong.wt,add,20240313,add force shutdown by pressing voldn+powerkey for 8s	
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	//+PERIDOT-359,zhaizhenhong.wt,add,20240313,add force shutdown by pressing voldn+powerkey for 8s	
	timer_setup(&com_key_timer,force_shutdown_timeout,0);
	INIT_WORK(&force_shutdown_w, kpd_force_shutdown_workfunc);
	//-PERIDOT-359,zhaizhenhong.wt,add,20240313,add force shutdown by pressing voldn+powerkey for 8s	
	return 0;
