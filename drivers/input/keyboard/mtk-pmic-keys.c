...
extern int pwrkey_dn, voldn_dn;
extern struct work_struct force_shutdown_w;
extern struct timer_list com_key_timer; 
				...
				int irq, void *data)
{
	...
	dev_dbg(info->keys->dev, "release key =%d using PMIC\n",
			info->keycode);
	//+PERIDOT-359,zhaizhenhong.wt,add,20240313,force shutdown by long oress voldn +power for 8s
	if (info->keycode == 116) {//release power key
                pwrkey_dn = 0;
		if (!pwrkey_dn){
...
static irqreturn_t mtk_pmic_keys_irq_handler_thread(int irq, void *data)
{
	...
	//+PERIDOT-359,zhaizhenhong.wt,add,20240313,force shutdown by long oress voldn +power for 8s
	if (info->keycode == 116) {//KEY_power
		pwrkey_dn = !!pressed;
		if (pwrkey_dn && voldn_dn) {
			pr_debug("double keys pressed:vol_dn + pwrkey\n");
