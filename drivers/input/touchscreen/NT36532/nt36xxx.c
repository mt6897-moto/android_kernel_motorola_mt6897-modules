...
#include <linux/power_supply.h> //for USB detect
...
#define DISP_ID0_DET (262+184)
#define DISP_ID1_DET (262+183)
//-P98300AA1-2658,fangzhihua.wt,add,20240228, add tp info
#if NVT_TOUCH_ESD_PROTECT
...
bool hallup_to_enable = true;//hall up :enable; hall down:disable
EXPORT_SYMBOL_GPL(hallup_to_enable);
static bool hall_last_cover_state = false;  //true: close keyboard; false:open keyboard
...
static struct pinctrl_state *nt36532_spi_cs_high;
static struct pinctrl_state *nt36532_spi_cs_low;
static struct pinctrl_state *nt36532_int_active;
static struct pinctrl_state *nt36532_int_suspend;
static struct pinctrl_state *nt36532_rst_active;
static struct pinctrl_state *nt36532_rst_suspend;
...
//static struct pinctrl_state *nt36532_1p8v_en_high;  //63
static struct pinctrl_state *nt36532_1p8v_en_low;
//static struct pinctrl_state *nt36532_bias_n_en_high;  //186
static struct pinctrl_state *nt36532_bias_n_en_low;
//static struct pinctrl_state *nt36532_bias_p_en_high;   //187
static struct pinctrl_state *nt36532_bias_p_en_low;
/* -PERIDOT-12531,fangzhihua.wt,20240918,add to fix current is biger than that needed 1-2mA after screenoff then close keyboard */


/*+chklst-98554,fangzhihua.wt,add, 20240330, add pinctrl ID0 to control VDD 1.8V*/
static struct pinctrl_state *nt36532_gpio184_low;
static struct pinctrl_state *nt36532_gpio184_high;
/*-chklst-98554,fangzhihua.wt,add, 20240330, add pinctrl ID0 to control VDD 1.8V*/
#endif
//-chklst-98554,fangzhihua.wt,add,20240338,add pinctrl to cs,irq,int
...
//+peridot-10940,fangzhihua.wt,20240611,fix TP no function after fast suspend/resume or resume with screen unlock
struct tag_bootmode {
	u32 size;
	u32 tag;
	u32 bootmode;
...
/*static*/ uint8_t bTouchIsAwake = 0;  //share with hall
EXPORT_SYMBOL_GPL(bTouchIsAwake);
...
//+chklst-98554,fangzhihua.wt,add,20240338,add pinctrl to cs,irq,int
#ifdef CONFIG_SPI_CS_ONTROL
void nvt_set_cs(u8 val);  //fangzhihua.wt
EXPORT_SYMBOL_GPL(nvt_set_cs);

/* +PERIDOT-12531,fangzhihua.wt,20240918,add to fix current is biger than that needed 1-2mA after screenoff then close keyboard */
void hall_close_tp_poweroff(void);   //screenoff, then close keyboard, hall to poweroff TP using pinctrl
EXPORT_SYMBOL_GPL(hall_close_tp_poweroff);
/* -PERIDOT-12531,fangzhihua.wt,20240918,add to fix current is biger than that needed 1-2mA after screenoff then close keyboard */

int nt36532_ts_gpio_active(void);
int nt36532_ts_gpio_suspend(void);
#endif
//-chklst-98554,fangzhihua.wt,add,20240338,add pinctrl to cs,irq,int
...
void get_tp_info(void)
{
	...
	nvt_get_fw_info();
	NVT_ERR("get_tp_info    gpio_184 = %d \n",gpio_184);
	NVT_ERR("get_tp_info    gpio_183 = %d \n",gpio_183);
	if (gpio_184==0){
		snprintf(tp_version_info,HARDWARE_MAX_ITEM_LONGTH,"[Vendor]:BOE, [TP-IC]:NT36532, [fw]:0x%02X", ts->fw_ver);
		hardwareinfo_set_prop(HARDWARE_TP,tp_version_info);
		pr_info("[%s]: tp_info:  %s\n", __func__, tp_version_info);
	} else {
		snprintf(tp_version_info,HARDWARE_MAX_ITEM_LONGTH,"[Vendor]:TIANMA, [TP-IC]:NT36532, [fw]:0x%02X", ts->fw_ver);
		hardwareinfo_set_prop(HARDWARE_TP,tp_version_info);
		pr_info("[%s]: tp_info:   %s\n", __func__, tp_version_info);
	}
}
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		if (gpio_184 == 0){
			NVT_LOG("%s  ,BOOT BOE !\n",__func__);
			nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME_BOE);
		}else{
			NVT_LOG("%s  ,BOOT TIANMA !\n",__func__);
			nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME_TIANMA);
		}
		//-P98300AA1-2658,fangzhihua.wt,change,20240304, distinguish 1st add 2nd  tp firmware
...
return:
	n.a.
*******************************************************/
uint32_t pen_battery = 0;  //penlink,fangzhihua.wt ,20240426,add patch
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
			...
			//printk("fzhtest input_x = %d         input_y=%d   ", input_x,input_y);
			if ((input_y > TOUCH_MAX_HEIGHT) || (input_x > TOUCH_MAX_WIDTH))
			{
				continue;
			}
			...
			//printk("fzhtest after x = %04X     y=%04X   ", input_x,input_y);
			//-peridot-867],fangzhihua.wt, 20240327, TP coord rotate 270 because camera rotated 90
...
*/
		...
		if (pen_format_id != 0xFF) {
			if (pen_format_id == 0x01) {
				...
				//peridot-867],fangzhihua.wt, 20240327, TP coord rotate 270 because camera rotated 90
				if ((pen_y > PEN_MAX_HEIGHT) || (pen_x > PEN_MAX_WIDTH)){
					goto XFER_ERROR;
				}
...
//+penlink,fangzhihua.wt ,20240426,add patch
				...
				NVT_LOG("pen_id 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X, checksum 0x%X, type 0x%X\n",
						point_data[67], point_data[68], point_data[69], point_data[70],
						point_data[71], point_data[72], point_data[73], point_data[74],
						point_data[75]);
#endif
				goto XFER_ERROR;
//-penlink,fangzhihua.wt ,20240426,add patch
...
//+penlink,fangzhihua.wt ,20240426,add patch
#if NVT_REPORT_PEN_MAC_ADDR
int ts_pen_mac_addr_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	int ret = 0;
...
//+chklst-98554,fangzhihua.wt,add,20240338,add pinctrl to cs,irq,int
#ifdef CONFIG_SPI_CS_ONTROL
static int nvt_cs_init_dts(struct nvt_ts_data *ts)
{
	int ret = 0;

	nt36532_pinctrl = devm_pinctrl_get(ts->client->controller->dev.parent);
	...
	nt36532_spi_cs_low = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_SPI_CS_LOW);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_SPI_CS_LOW, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}
	nt36532_spi_cs_high = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_SPI_CS_HIGH);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_SPI_CS_HIGH, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}

	nt36532_int_active = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_INT_ACTIVE);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_INT_ACTIVE, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}

	nt36532_int_suspend = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_INT_SUSPEND);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_INT_SUSPEND, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}

	nt36532_rst_active = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_RST_ACTIVE);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_RST_ACTIVE, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}

	nt36532_rst_suspend = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_RST_SUSPEND);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_RST_SUSPEND, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}

/* +PERIDOT-12531,fangzhihua.wt,20240918,add to fix current is biger than that needed 1-2mA after screenoff then close keyboard */
	/*nt36532_1p8v_en_high = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_1P8V_EN_HIGH);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_1P8V_EN_HIGH, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}*/
	nt36532_1p8v_en_low = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_1P8V_EN_LOW);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_1P8V_EN_LOW, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}
	/*nt36532_bias_n_en_high = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_BIASN_EN_HIGH);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_BIASN_EN_HIGH, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}*/
	nt36532_bias_n_en_low = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_BIASN_EN_LOW);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_BIASN_EN_LOW, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}
	/*nt36532_bias_p_en_high = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_BIASP_EN_HIGH);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_BIASP_EN_HIGH, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}*/
	nt36532_bias_p_en_low = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_BIASP_EN_LOW);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_BIASP_EN_LOW, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	}
/* -PERIDOT-12531,fangzhihua.wt,20240918,add to fix current is biger than that needed 1-2mA after screenoff then close keyboard */
...
/*+chklst-98554,fangzhihua.wt,add, 20240330, add pinctrl ID0 to control VDD 1.8V*/
	nt36532_gpio184_low = pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_GPIO184_LOW);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_GPIO184_LOW, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	};
	nt36532_gpio184_high= pinctrl_lookup_state(
				nt36532_pinctrl, PINCTRL_STATE_GPIO184_HIGH);
	if (IS_ERR_OR_NULL(nt36532_pinctrl)) {
		ret = PTR_ERR(nt36532_pinctrl);
		NVT_ERR("Failed to get pinctrl state:%s, r:%d",PINCTRL_STATE_GPIO184_HIGH, ret);
		nt36532_pinctrl = NULL;
		goto err_pinctrl_failed;
	};
/*-chklst-98554,fangzhihua.wt,add, 20240330, add pinctrl ID0 to control VDD 1.8V*/
...
void nvt_set_cs(u8 val)
{
	NVT_LOG("nvt_set_cs =%d :0 is low, 1 is high \n", val);

	if (val){
		pinctrl_select_state(nt36532_pinctrl, nt36532_spi_cs_high);
	}else{
		pinctrl_select_state(nt36532_pinctrl, nt36532_spi_cs_low);
/*chklst-98554,fangzhihua.wt,add, 20240330, add pinctrl ID0 to control VDD 1.8V*/
		pinctrl_select_state(nt36532_pinctrl, nt36532_gpio184_low);
	}
	//return 0;
}

/* +PERIDOT-12531,fangzhihua.wt,20240918,add to fix current is biger than that needed 1-2mA after screenoff then close keyboard */
void hall_close_tp_poweroff(void)
{
	int err = 0;
	if (nt36532_pinctrl) {
		NVT_LOG(" start %s  \n", __func__);
		err = pinctrl_select_state(nt36532_pinctrl, nt36532_1p8v_en_low);
		if (err < 0) {
			NVT_ERR("Failed to select nt36532_1p8v_en_low, ret:%d", err);
		}
		err = pinctrl_select_state(nt36532_pinctrl, nt36532_bias_n_en_low);
		if (err < 0) {
			NVT_ERR("Failed to select nt36532_bias_n_en_low, ret:%d", err);
		}
		err = pinctrl_select_state(nt36532_pinctrl, nt36532_bias_p_en_low);
		if (err < 0) {
			NVT_ERR("Failed to select nt36532_bias_p_en_low, ret:%d", err);
		}
		msleep(10);
		err = pinctrl_select_state(nt36532_pinctrl, nt36532_spi_cs_low);
		if (err < 0) {
			NVT_ERR("Failed to select nt36532_spi_cs_low, ret:%d", err);
		}
		if (nt36532_ts_gpio_suspend()< 0)
			NVT_LOG("Failed to select rst/eint suspend state");
	}
}
/* -PERIDOT-12531,fangzhihua.wt,20240918,add to fix current is biger than that needed 1-2mA after screenoff then close keyboard */
...
int nt36532_ts_gpio_active(void)
{
	...
	if (nt36532_pinctrl) {
		ret = pinctrl_select_state(nt36532_pinctrl,nt36532_int_active);

		if (ret < 0) {
			NVT_ERR("Failed to select int active state, ret:%d", ret);
			goto err_resume_pinctrl;
		}
		ret = pinctrl_select_state(nt36532_pinctrl,nt36532_rst_active);
		if (ret < 0) {
			NVT_ERR("Failed to select rst active state, ret:%d", ret);
			goto err_resume_pinctrl;
		}
	}
err_resume_pinctrl:
...
int nt36532_ts_gpio_suspend(void)
{
	...
	if (nt36532_pinctrl) {
		ret  = pinctrl_select_state(nt36532_pinctrl, nt36532_int_suspend);
		if (ret < 0) {
			NVT_ERR("Failed to select int suspend state, ret:%d", ret);
			goto err_suspend_pinctrl;
		}
		ret  = pinctrl_select_state(nt36532_pinctrl,nt36532_rst_suspend);
		if (ret < 0) {
			NVT_ERR("Failed to select rst suspend state, ret:%d", ret);
			goto err_suspend_pinctrl;
		}
	}
err_suspend_pinctrl:
...
//-chklst-98554,fangzhihua.wt,add,20240338,add pinctrl to cs,irq,int
//+peridot-2133,fangzhihua.wt ,change,20240419, fix for TP chanrging state
static int nvt_charger_usb_statte(void)
{
	int ret = 0;
	...
	if (psy) {
		ret = power_supply_get_property(psy,POWER_SUPPLY_PROP_USB_TYPE,&prop);// PRESENT  POWER_SUPPLY_PROP_ONLINE
		if (ret < 0) {
			NVT_ERR("Couldn't get POWER_SUPPLY_PROP_USB_TYPE rc=%d\n", ret);
		} else {
			ts->usb_plug_status = prop.intval;
			//NVT_LOG("boot check usb_plug_status = %d\n", ts->usb_plug_status);
		}
	}
	//NVT_LOG("nvt_charger_usb_statte  ts->usb_plug_status = %d\n",ts->usb_plug_status);
...
{
	int ret;
        int usb_plug_old_status = ts->usb_plug_status;

	//NVT_LOG("nvt_charger_notify_work  start \n");
	...
	if (!bTouchIsAwake){
		NVT_ERR("%s: TP is suspend! \n", __func__);
		return;
	}
	ret = nvt_charger_usb_statte();
	if (ret < 0)
		NVT_ERR("get nvt_charger_usb_statte  error \n");
	else{
		if ((usb_plug_old_status != ts->usb_plug_status)||need_write_charger_state_flag)
		{
			if (ts->usb_plug_status == 0){
			...
			need_write_charger_state_flag = false;
		};
	}
	queue_delayed_work(ts->nvt_charger_notify_wq, &ts->charger_notify_work,
			msecs_to_jiffies(1500));
	//NVT_LOG("nvt_charger_notify_work  end \n");
}
#if 0
static int nvt_charger_notifier_callback(struct notifier_block *nb,unsigned long val, void *v)
{
	...
	NVT_ERR("nvt_charger_notifier_callback  step1 \n");

	if (ts->fw_update_stat != 1)
		return 0;

	NVT_ERR("nvt_charger_notifier_callback  step2 \n");
	...
	NVT_ERR("nvt_charger_notifier_callback  step4  	ts->usb_plug_status = %d\n",ts->usb_plug_status);
	return 0;
}
#endif
void nvt_charger_init(void)
{
	...
	NVT_LOG("nvt_charger_init start \n");

	INIT_DELAYED_WORK(&ts->charger_notify_work, nvt_charger_notify_work);
	ts->nvt_charger_notify_wq = alloc_workqueue("nvt_charger_wq", WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
	...
	NVT_LOG("nvt_charger_init end \n");
}
//-peridot-2133,fangzhihua.wt ,change,20240419, fix for TP chanrging state

//+peridot-10940,fangzhihua.wt,20240611,fix TP no function after fast suspend/resume or resume with screen unlock
static int tp_bootmode;
static void mtk_tp_charger_parse_dt(struct device *dev)
{
	struct device_node *boot_node = NULL;
	struct tag_bootmode *tag = NULL;

	boot_node = of_parse_phandle(dev->of_node, "tp_bootmode", 0);
	if (!boot_node)
		NVT_LOG("%s: failed to get boot mode phandle\n", __func__);
	else {
		tag = (struct tag_bootmode *)of_get_property(boot_node,
							"atag,boot", NULL);
		if (!tag)
			NVT_LOG("%s: failed to get atag,boot\n", __func__);
		else {
			NVT_LOG("%s: size:0x%x tag:0x%x bootmode:0x%x boottype:0x%x\n",
				__func__, tag->size, tag->tag,
				tag->bootmode, tag->boottype);
			tp_bootmode = tag->bootmode;
		}
	}
}
//-peridot-10940,fangzhihua.wt,20240611,fix TP no function after fast suspend/resume or resume with screen unlock
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	NVT_LOG("start\n");
//+peridot-10940,fangzhihua.wt,20240611,fix TP no function after fast suspend/resume or resume with screen unlock
	mtk_tp_charger_parse_dt(&client->dev);

	/* 8 = KERNEL_POWER_OFF_CHARGING_BOOT */
	/* 9 = LOW_POWER_OFF_CHARGING_BOOT */
	if ((tp_bootmode==8)||(tp_bootmode==9))
	{
		NVT_ERR("power off charging and low power charger, TP not load\n");
		return -1;
	}
//-peridot-10940,fangzhihua.wt,20240611,fix TP no function after fast suspend/resume or resume with screen unlock
...
//+chklst-98554,fangzhihua.wt,add,20240338,add pinctrl to cs,irq,int
#ifdef CONFIG_SPI_CS_ONTROL
	ret = nvt_cs_init_dts(ts);
	if (ret) {
		NVT_ERR("gpio cs config error!\n");
	}
	nvt_set_cs(1);
#endif
//-chklst-98554,fangzhihua.wt,add,20240338,add pinctrl to cs,irq,int
...
//-peridot-2133,fangzhihua.wt ,change,20240419, fix for TP chanrging state
...
#elif IS_ENABLED(NVT_MTK_DRM_NOTIFY)   //use here
	...
	ts->probe_state = 1;
#endif
//+enlink,fangzhihua.wt ,20240426,add patch
#if IS_ENABLED(NVT_PLATFORM_DRIVER)
	/* ts platform deivce */
...
#if IS_ENABLED(NVT_DRM_PANEL_NOTIFY) || IS_ENABLED(NVT_MSM_DRM_NOTIFY) || IS_ENABLED(NVT_FB_NOTIFY) || IS_ENABLED(NVT_EARLYSUSPEND_NOTIFY) || IS_ENABLED(NVT_QCOM_PANEL_EVENT_NOTIFY) || IS_ENABLED(NVT_MTK_DRM_NOTIFY)
...
//+peridot-2133,fangzhihua.wt ,change,20240419, fix for TP chanrging state
  	if(ts->nvt_charger_notify_wq)
  	{
  		cancel_delayed_work_sync(&ts->charger_notify_work);
...
static int32_t nvt_ts_suspend(struct device *dev)
{
	...
	NVT_LOG("%s,fzh, hallup_to_enable=%d   bDoubleWakeupflag=%d \n", __func__, hallup_to_enable,bDoubleWakeupflag);//hallup_to_enable=0:open keyboard
	hall_last_cover_state = hallup_to_enable;//peridot-12531,fangzhihua.wt,20240822,fix large current after pogokeyboard close when TP gesture wakeup function is on.

#if !WAKEUP_GESTURE
	nvt_irq_enable(false);
#else
//+peridot-12531,fangzhihua.wt,20240822,fix large current after pogokeyboard close when TP gesture wakeup function is on.
	if ((!bDoubleWakeupflag)||(!hallup_to_enable))
//-peridot-12531,fangzhihua.wt,20240822,fix large current after pogokeyboard close when TP gesture wakeup function is on.
		nvt_irq_enable(false);
#endif
...
//+peridot-12531,fangzhihua.wt,20240822,fix large current after pogokeyboard close when TP gesture wakeup function is on.
	if (bDoubleWakeupflag && !hallup_to_enable){
        	set_boe_tp_wakeup_flag(hallup_to_enable);
        	set_tm_tp_wakeup_flag(hallup_to_enable);
        	set_tm_paper_tp_wakeup_flag(hallup_to_enable);
	}
	if (bDoubleWakeupflag && (hallup_to_enable)){
//-peridot-12531,fangzhihua.wt,20240822,fix large current after pogokeyboard close when TP gesture wakeup function is on.
...
//+peridot-12531,fangzhihua.wt,20240822,fix large current after pogokeyboard close when TP gesture wakeup function is on.
	if ((!bDoubleWakeupflag)||!hallup_to_enable){
		NVT_LOG("%s,fzh2, hallup_to_enable=%d   bDoubleWakeupflag=%d \n", __func__, hallup_to_enable,bDoubleWakeupflag);
//-peridot-12531,fangzhihua.wt,20240822,fix large current after pogokeyboard close when TP gesture wakeup function is on.
		nvt_set_cs(0);
		if (nt36532_ts_gpio_suspend()< 0)
			NVT_LOG("Failed to select int suspend state");
	}
#else
	nvt_set_cs(0);
	if (nt36532_ts_gpio_suspend()< 0)
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	NVT_LOG("%s,fzh...., hall_last_cover_state=%d   bDoubleWakeupflag=%d \n", __func__, hall_last_cover_state,bDoubleWakeupflag);
...
//peridot-12531,fangzhihua.wt,20240822,fix large current after pogokeyboard close when TP gesture wakeup function is on.
	if (bDoubleWakeupflag){
        	set_boe_tp_wakeup_flag(1);
        	set_tm_tp_wakeup_flag(1);
        	set_tm_paper_tp_wakeup_flag(1);
	}
/* +PERIDOT-12531,fangzhihua.wt,20240918,add to fix current is biger than that needed 1-2mA after screenoff then close keyboard */
	/*if (!bDoubleWakeupflag||!hall_last_cover_state ){
		nvt_set_cs(1);
		if (nt36532_ts_gpio_active()< 0)
			NVT_LOG("Failed to select int suspend state");
	}
#else
	nvt_set_cs(1);
...
//- fangzhihua.wt,20240429,add Lenove DDA patch
	...
	NVT_LOG("start\n");
//+P98300AA1-2658,fangzhihua.wt,add,20240228, add tp info
	gpio_184 = gpio_get_value(DISP_ID0_DET);
	gpio_183 = gpio_get_value(DISP_ID1_DET);
	NVT_LOG("gpio_184 = %d,  gpio_183 = %d ", gpio_184,gpio_183);
//-P98300AA1-2658,fangzhihua.wt,add,20240228, add tp info
	//---add spi driver---
