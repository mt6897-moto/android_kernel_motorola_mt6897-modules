 ...
 *
 * $Revision: 109657 $
 * $Date: 2022-12-07 16:24:36 +0800 (週三, 07 十二月 2022) $
 *
...
//+P98300AA1-2653,fangzhihua.wt,mod,20240321,change dir for FTM TP self test, result file add time 
#include <linux/rtc.h>
...
#if 1//+P98300AA1-2653,fangzhihua.wt,mod,20240321,change dir for FTM TP self test, result file add time 
#define SELF_TEST_CSV_DIR         "/data/vendor/fac_sources/FTM_AP/"
#define SHORT_TEST_CSV_FILE       "ShortTest.csv"
#define OPEN_TEST_CSV_FILE        "OpenTest.csv"
#define FW_RAWDATA_CSV_FILE       "FWRawdataTest.csv"
#define FW_CC_CSV_FILE            "FWCCTest.csv"
#define NOISE_TEST_CSV_FILE       "NoiseTest.csv"
#define PEN_FW_RAW_TEST_CSV_FILE  "PenFWRawTest.csv"
#define PEN_NOISE_TEST_CSV_FILE   "PenNoiseTest.csv"
#else
#define SHORT_TEST_CSV_FILE       "/data/vendor/fac_sources/FTM_AP/ShortTest.csv"
#define OPEN_TEST_CSV_FILE        "/data/vendor/fac_sources/FTM_AP/OpenTest.csv"
#define FW_RAWDATA_CSV_FILE       "/data/vendor/fac_sources/FTM_AP/FWRawdataTest.csv"
#define FW_CC_CSV_FILE            "/data/vendor/fac_sources/FTM_AP/FWCCTest.csv"
#define NOISE_TEST_CSV_FILE       "/data/vendor/fac_sources/FTM_AP/NoiseTest.csv"
#define PEN_FW_RAW_TEST_CSV_FILE  "/data/vendor/fac_sources/FTM_AP/PenFWRawTest.csv"
#define PEN_NOISE_TEST_CSV_FILE   "/data/vendor/fac_sources/FTM_AP/PenNoiseTest.csv"
#endif
//-P98300AA1-2653,fangzhihua.wt,mod,20240321,change dir for FTM TP self test, result file add time 
...
//+P98300AA1-2653,fangzhihua.wt,mod,20240321,change dir for FTM TP self test, result file add time 
static char nvt_selftest_time[40]; //fangzhihua. openshort test result
static char SHORT_TEST_CSV_FILE_PATH[128];
static char OPEN_TEST_CSV_FILE_PATH[128];
static char FW_RAWDATA_CSV_FILE_PATH[128];
static char FW_CC_CSV_FILE_PATH[128];
static char NOISE_TEST_CSV_FILE_PATH[128];
static char PEN_FW_RAW_TEST_CSV_FILE_PATH[128];
static char PEN_NOISE_TEST_CSV_FILE_PATH[128];
//-P98300AA1-2653,fangzhihua.wt,mod,20240321,change dir for FTM TP self test, result file add time 
...
//+checklist 931, fangzhihua.wt, 20240222, change self_test result volume for FTM test
static struct proc_dir_entry *NVT_proc_android_toucht_entry;
//-checklist 931, fangzhihua.wt, 20240222, change self_test result volume for FTM test
...
//+fangzhihua.wt,add,20240405, fix GKI bootimage TP fail
extern __attribute__ ((weak)) struct file *filp_open(const char *filename, int flags, umode_t mode);
extern __attribute__ ((weak)) ssize_t kernel_write(struct file *file, const void *buf, size_t count,loff_t *pos);
//-fangzhihua.wt,add,20240405, fix GKI bootimage TP fail
/*******************************************************
Description:
...
static int32_t nvt_read_baseline(int32_t *xdata)
{
	...
	printk("%s:    filename=%s \n", __func__,FW_RAWDATA_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV file
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	ret = nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, FW_RAWDATA_CSV_FILE_PATH, 0);
	if (ret < 0) {
...
static int32_t nvt_read_CC(int32_t *xdata)
{
	...
	printk("%s:     filename =%s \n", __func__,FW_CC_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV file
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	ret = nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, FW_CC_CSV_FILE_PATH, 0);
	if (ret < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
...
static int32_t nvt_read_pen_baseline(void)
{
	...
	// Save Rawdata to CSV file
	printk("%s:RawData_PenTipX_Raw       filename =%s  \n", __func__,PEN_FW_RAW_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	if (nvt_save_rawdata_to_csv(RawData_PenTipX_Raw, ts->x_num, ts->y_gang_num, PEN_FW_RAW_TEST_CSV_FILE_PATH, csv_output_offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	...
	printk("%s:RawData_PenTipY_Raw      filename =%s   \n", __func__,PEN_FW_RAW_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	ret = nvt_save_rawdata_to_csv(RawData_PenTipY_Raw, ts->x_gang_num, ts->y_num, PEN_FW_RAW_TEST_CSV_FILE_PATH, csv_output_offset);
	...
	printk("%s:RawData_PenRingX_Raw     filename =%s   \n", __func__ ,PEN_FW_RAW_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	ret = nvt_save_rawdata_to_csv(RawData_PenRingX_Raw, ts->x_num, ts->y_gang_num, PEN_FW_RAW_TEST_CSV_FILE_PATH, csv_output_offset);
	if (ret < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	...
	printk("%s:RawData_PenRingY_Raw     filename =%s  \n", __func__,PEN_FW_RAW_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	ret = nvt_save_rawdata_to_csv(RawData_PenRingY_Raw, ts->x_gang_num, ts->y_num, PEN_FW_RAW_TEST_CSV_FILE_PATH, csv_output_offset);
	if (ret < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	printk("%s:RawData_Diff_Max:       filename =%s  \n", __func__,NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV file
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	if (nvt_save_rawdata_to_csv(RawData_Diff_Max, X_Channel, Y_Channel, NOISE_TEST_CSV_FILE_PATH, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
#if TOUCH_KEY_NUM > 0
	...
	printk("%s:RawData_Diff_Min:     filename =%s  \n", __func__,NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV file
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	if (nvt_save_rawdata_to_csv(RawData_Diff_Min, X_Channel, Y_Channel, NOISE_TEST_CSV_FILE_PATH, rawdata_diff_min_offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
#else /* #if NVT_SAVE_TEST_DATA_IN_FILE */
	...
	if (ts->pen_support) {
		printk("%s:RawData_PenTipX_DiffMax:      filename =%s \n", __func__,PEN_NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
		//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
		if (nvt_save_rawdata_to_csv(RawData_PenTipX_DiffMax, ts->x_num, ts->y_gang_num, PEN_NOISE_TEST_CSV_FILE_PATH, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		...
		printk("%s:RawData_PenTipX_DiffMin:      filename =%s  \n", __func__,PEN_NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
		//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
		if (nvt_save_rawdata_to_csv(RawData_PenTipX_DiffMin, ts->x_num, ts->y_gang_num, PEN_NOISE_TEST_CSV_FILE_PATH, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
		...
		printk("%s:RawData_PenTipY_DiffMax:        filename =%s  \n", __func__,PEN_NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
		//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
		if (nvt_save_rawdata_to_csv(RawData_PenTipY_DiffMax, ts->x_gang_num, ts->y_num, PEN_NOISE_TEST_CSV_FILE_PATH, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		...
		printk("%s:RawData_PenTipY_DiffMin:       filename =%s   \n", __func__,PEN_NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
		//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
		if (nvt_save_rawdata_to_csv(RawData_PenTipY_DiffMin, ts->x_gang_num, ts->y_num, PEN_NOISE_TEST_CSV_FILE_PATH, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_num * ts->x_gang_num * 7 + ts->y_num * 2;
		...
		printk("%s:RawData_PenRingX_DiffMax:      filename =%s    \n", __func__,PEN_NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
		//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
		if (nvt_save_rawdata_to_csv(RawData_PenRingX_DiffMax, ts->x_num, ts->y_gang_num, PEN_NOISE_TEST_CSV_FILE_PATH, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
		...
		printk("%s:RawData_PenRingX_DiffMin:     filename =%s   \n", __func__,PEN_NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
		//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
		if (nvt_save_rawdata_to_csv(RawData_PenRingX_DiffMin, ts->x_num, ts->y_gang_num, PEN_NOISE_TEST_CSV_FILE_PATH, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
		...
		printk("%s:RawData_PenRingY_DiffMax:    filename =%s  \n", __func__,PEN_NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
		//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
		if (nvt_save_rawdata_to_csv(RawData_PenRingY_DiffMax, ts->x_gang_num, ts->y_num, PEN_NOISE_TEST_CSV_FILE_PATH, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_num * ts->x_gang_num * 7 + ts->y_num * 2;
		...
		printk("%s:RawData_PenRingY_DiffMin:     filename =%s   \n", __func__,PEN_NOISE_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
		//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
		ret = nvt_save_rawdata_to_csv(RawData_PenRingY_DiffMin, ts->x_gang_num, ts->y_num, PEN_NOISE_TEST_CSV_FILE_PATH, csv_pen_noise_offset);
		if (ret < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
...
static int32_t nvt_read_fw_open(int32_t *xdata)
{
	...
	printk("%s:RawData_Open        filename =%s  \n", __func__,OPEN_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save RawData to CSV file
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	ret = nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, OPEN_TEST_CSV_FILE_PATH, 0);
	if (ret < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	printk("%s:RawData_Short     filename =%s   \n", __func__,SHORT_TEST_CSV_FILE_PATH);
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV fil
	//P98300AA1-2653,fangzhihua.wt,mod,20240321,change file name with test time
	ret = nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, SHORT_TEST_CSV_FILE_PATH, 0);
...
//+P98300AA1-2653,fangzhihua.wt,mod,20240321,change dir for FTM TP self test, result file add time 
	...
	memset(SHORT_TEST_CSV_FILE_PATH,'\0',sizeof(SHORT_TEST_CSV_FILE_PATH));
	memset(OPEN_TEST_CSV_FILE_PATH,'\0',sizeof(OPEN_TEST_CSV_FILE_PATH));
	memset(FW_RAWDATA_CSV_FILE_PATH,'\0',sizeof(FW_RAWDATA_CSV_FILE_PATH));
	memset(FW_CC_CSV_FILE_PATH,'\0',sizeof(FW_CC_CSV_FILE_PATH));
	memset(NOISE_TEST_CSV_FILE_PATH,'\0',sizeof(NOISE_TEST_CSV_FILE_PATH));
	memset(PEN_FW_RAW_TEST_CSV_FILE_PATH,'\0',sizeof(PEN_FW_RAW_TEST_CSV_FILE_PATH));
	memset(PEN_NOISE_TEST_CSV_FILE_PATH,'\0',sizeof(PEN_NOISE_TEST_CSV_FILE_PATH));

	memset(nvt_selftest_time,'\0',sizeof(nvt_selftest_time));
	ktime_get_ts64(&ktv);

	ktv.tv_sec -= sys_tz.tz_minuteswest*60;
	rtc_time64_to_tm(ktv.tv_sec,&rtc_tm);
	snprintf(nvt_selftest_time,sizeof(nvt_selftest_time),"%04d%02d%02d_%02d%02d%02d",rtc_tm.tm_year+1900,
		rtc_tm.tm_mon+1,rtc_tm.tm_mday,rtc_tm.tm_hour,rtc_tm.tm_min,rtc_tm.tm_sec);

	snprintf(SHORT_TEST_CSV_FILE_PATH,128,"%s%s%s%s",SELF_TEST_CSV_DIR,nvt_selftest_time,"_",SHORT_TEST_CSV_FILE);
	snprintf(OPEN_TEST_CSV_FILE_PATH,128,"%s%s%s%s",SELF_TEST_CSV_DIR,nvt_selftest_time,"_",OPEN_TEST_CSV_FILE);
	snprintf(FW_RAWDATA_CSV_FILE_PATH,128,"%s%s%s%s",SELF_TEST_CSV_DIR,nvt_selftest_time,"_",FW_RAWDATA_CSV_FILE);
	snprintf(FW_CC_CSV_FILE_PATH,128,"%s%s%s%s",SELF_TEST_CSV_DIR,nvt_selftest_time,"_",FW_CC_CSV_FILE);
	snprintf(NOISE_TEST_CSV_FILE_PATH,128,"%s%s%s%s",SELF_TEST_CSV_DIR,nvt_selftest_time,"_",NOISE_TEST_CSV_FILE);
	snprintf(PEN_FW_RAW_TEST_CSV_FILE_PATH,128,"%s%s%s%s",SELF_TEST_CSV_DIR,nvt_selftest_time,"_",PEN_FW_RAW_TEST_CSV_FILE);
	snprintf(PEN_NOISE_TEST_CSV_FILE_PATH,128,"%s%s%s%s",SELF_TEST_CSV_DIR,nvt_selftest_time,"_",PEN_NOISE_TEST_CSV_FILE);
//-P98300AA1-2653,fangzhihua.wt,mod,20240321,change dir for FTM TP self test, result file add time 
	...
	if (gpio_184 == 0){
		NVT_LOG("%s  ,MP BOE !\n",__func__);
		ret = nvt_update_firmware(MP_UPDATE_FIRMWARE_NAME_BOE);
	}else{
		NVT_LOG("%s  ,MP TIANMA !\n",__func__);
		ret = nvt_update_firmware(MP_UPDATE_FIRMWARE_NAME_TIANMA);
	}
	if (ret <0 ){
	...
	if(of_property_read_bool(np, "novatek,mp-support-dt")) {
		...
		if (nvt_mp_parse_dt(np, mpcriteria)) {
			//---Download Normal FW---
			//P8300AA1-2658,fangzhihua.wt,change,20240304, distinguish 1st add 2nd  tp firmware
			if (gpio_184 == 0){
				NVT_LOG("%s  ,BOOT BOE !\n",__func__);
...
{
	//+chklist 931, fangzhihua.wt, 20240222, change self_test volume for for FTM test  
	NVT_proc_android_toucht_entry = proc_mkdir("android_touch",NULL);
	if (NVT_proc_android_toucht_entry == NULL)
	{
		NVT_ERR("create /proc/android_touch Failed!\n");
		return -1;
	}

	NVT_proc_selftest_entry = proc_create("self_test", 0444, NVT_proc_android_toucht_entry, &nvt_selftest_fops); //nvt_selftest
	//-chklist 931, fangzhihua.wt, 20240222, change self_test volume for for FTM test  
