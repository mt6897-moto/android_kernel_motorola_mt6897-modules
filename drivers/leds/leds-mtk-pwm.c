		...
		int brightness, unsigned int params, unsigned int params_flag)
{
	...
	if ((brightness < 52) && (brightness > 13)) {
                brightness = brightness - 13;
	} else if ((brightness > 0) && (brightness <= 13)) {
		brightness = 1;
	}

	pr_debug("led_pwm_set is %d, %d, %lld", max, brightness, led_dat->pwmstate.period);
	duty *= brightness;
	do_div(duty, max);
