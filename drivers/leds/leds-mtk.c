...
#define BACKLIGHT_UP 10
#define BACKLIGHT_DOWN 10
		...
		unsigned int params, unsigned int params_flag)
{
	...
	int ret = 0;
	int brightness_offset = 0, brightness_flag = 0, i;
	...
	if (((brightness - led_dat->hw_brightness) > 1000) && (led_dat->hw_brightness != 0)) {
		brightness_flag = brightness;
                brightness_offset = brightness - led_dat->hw_brightness;
		brightness = led_dat->hw_brightness;
		for (i=0; i <= BACKLIGHT_UP; i++) {
			brightness +=  brightness_offset/BACKLIGHT_UP;
			mdelay(10);
			if (brightness >= brightness_flag) {
				brightness = brightness_flag;
				break;
			}
			led_dat->mtk_hw_brightness_set(led_dat, brightness, params, params_flag);
		}
        } else if (((led_dat->hw_brightness - brightness) > 1000) && (brightness != 0)) {
		brightness_flag = brightness;
                brightness_offset = led_dat->hw_brightness-brightness;
		brightness = led_dat->hw_brightness;
                for (i=0; i <= BACKLIGHT_DOWN; i++) {
                        brightness -= brightness_offset/BACKLIGHT_DOWN;
			mdelay(10);
                        if (brightness <= brightness_flag) {
                                brightness = brightness_flag;
                                break;
		...
		unsigned int params, unsigned int params_flag)
{
	...
	if (!led_dat->conf.aal_enable) {
		...
		pr_debug("mtk_leds_brightness_set wanwen1 set hw brightness: %d",level);
	}
					  ...
					  enum led_brightness brightness)
{
	...
	if (!led_conf->aal_enable) {
		...
		pr_debug("wanwen1 ====== set hw brightness: %d",trans_level);
...
int mt_leds_parse_dt(struct mt_led_data *mdev, struct fwnode_handle *fwnode)
{
	...
	if (!ret) {
		...
		mdev->conf.cdev.brightness = 1945; //Peridot-32,zhangshaoxiong1.wt,modify,20240530,modify kernel pwm to 47%
	}
