...
static dev_t afw_devid;
static struct class *afw_class;
static struct cdev afw_cdev;
...
afw_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case AFW_CMD_GET_DEF_RULE:
		ret = afw_get_def_rule(&req);
		if(ret) {
			printk("%s[%d]: afw_get_def_rule failed, %d\n", __FILE__, __LINE__, ret);
			goto out;
		}
		ret = copy_to_user((void *)arg, &req, sizeof(req));
		if(ret) {
	...
	case AFW_CMD_FLUSH_RULE_EX:
		ret = afw_flush_rule_ex(&req);
		if(ret) {
			printk("%s[%d]: afw_flush_rule_ex failed, %d\n", __FILE__, __LINE__, ret);
			goto out;
		}
		goto out;
...
static struct file_operations afw_file_opts = {
	owner: THIS_MODULE,
...
match:
	...
	if(afw_conf.tcp_reset && ret == NF_DROP && iph->nexthdr == IPPROTO_TCP) {
		if(state && state->sk && state->sk->sk_state == TCP_SYN_SENT) {
			afw_stat.tcp_reset_ipv6_num ++;
			ret |= (ECONNREFUSED << NF_VERDICT_QBITS);
		}
	}
out:
...
static const struct nf_hook_ops afw_hook_ops[] = {
	{
...
static int
afw_probe(struct platform_device *pdev)
{
...
static int
afw_remove(struct platform_device *pdev)
{
...
afw_init(void)
{
	...
	ret = afw_rule_init();
	if(ret) {
		printk("%s[%d]: afw_rule_init failed, %d\n", __FILE__, __LINE__, ret);
		goto out_rfc;
	}
	...
	ret = afw_proc_init();
	if(ret) {
		printk("%s[%d]: afw_proc_init failed, %d\n", __FILE__, __LINE__, ret);
		goto out_rule;
	}
	pr_info("[AFW] afw afw_proc_init returned. ret: %d\n", ret);

	#if 0
	ret = register_chrdev(AFW_DEV_MAJOR, AFW_DEV_NAME, &afw_file_opts);
	if(ret) {
		printk("%s[%d]: register_chrdev failed, %d\n", __FILE__, __LINE__, ret);
		goto out_proc;
	}
	#endif

	ret = alloc_chrdev_region(&afw_devid, 0, 1, AFW_DEV_NAME);
	if(ret) {
		pr_info("%s[%d]: alloc_chrdev_region failed, ret[%d]\n", __FILE__, __LINE__, ret);
		goto out_proc;
	}
	pr_info("[AFW] afw alloc_chrdev_region returned. ret: %d\n", ret);

	ret = platform_driver_register(&afw_driver);
	if(ret) {
		pr_info("%s[%d]: platform_driver_register failed, ret[%d]\n", __FILE__, __LINE__, ret);
		goto out_region;
	}
	pr_info("[AFW] afw platform_driver_register returned. ret: %d\n", ret);

	afw_class = class_create(THIS_MODULE, AFW_DEV_NAME);
	if(IS_ERR(afw_class)) {
		pr_info("%s[%d]: class_create failed, afw_class[%p]\n", __FILE__, __LINE__, afw_class);
		ret = -ENOMEM;
		goto out_driver;
	}

	class_dev = device_create(afw_class, NULL, afw_devid, NULL, AFW_DEV_NAME);
	if(IS_ERR(class_dev)) {
		pr_info("%s[%d]: device_create failed, class_dev[%p]\n", __FILE__, __LINE__, class_dev);
		ret = -ENOMEM;
		goto out_class;
	}

	cdev_init(&afw_cdev, &afw_file_opts);
	afw_cdev.owner = THIS_MODULE;

	ret = cdev_add(&afw_cdev, MKDEV(MAJOR(afw_devid), 0), 1);
	if(ret < 0) {
		pr_info("%s[%d]: cdev_add failed, ret[%d]\n", __FILE__, __LINE__, ret);
		goto out_device;
	}

	ret = nf_register_net_hooks(&init_net, afw_hook_ops, ARRAY_SIZE(afw_hook_ops));
	if(ret < 0) {
		printk("%s[%d]: nf_register_net_hooks failed, %d\n", __FILE__, __LINE__, ret);
		goto out_cdev;
	}
...
out_cdev:
	cdev_del(&afw_cdev);
out_device:
	device_destroy(afw_class, afw_devid);
out_class:
	class_destroy(afw_class);
out_driver:
	platform_driver_unregister(&afw_driver);
out_region:
	unregister_chrdev_region(afw_devid, 1);
//out_dev:
...
afw_cleanup(void)
{
	nf_unregister_net_hooks(&init_net, afw_hook_ops, ARRAY_SIZE(afw_hook_ops));
