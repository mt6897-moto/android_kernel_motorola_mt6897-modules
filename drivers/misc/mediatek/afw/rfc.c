...
rfc_init_rule(rfc_rule_st *rule, int max_rule_num, int fu8_num, int fu16_num)
{
	...
	if(fu8_num > MAX_FIELD_NUM_U8 || fu16_num > MAX_FIELD_NUM_U16) {
		printk("%s[%d]: check parameter failed, fu8_num[%d], fu16_num[%d]\n", __FILE__, __LINE__, fu8_num, fu16_num);
		ret = -1;
		goto out;
	}
	...
	if(fu8_num) {
		for(i = 0; i < fu8_num; i ++) {
			...
			if(!rule->fu8[i]) {
				printk("%s[%d]: rfc_malloc_field failed\n", __FILE__, __LINE__);
				ret = -1;
				goto out_free;
			}
		}
...
rfc_build_cbm_u8(int chunk_idx, int rule_idx, rfc_field_u8_st *fu8)
{
	...
	if(ret) {
		printk("%s[%d]: rfc_build_cbm failed, %d\n", __FILE__, __LINE__, ret);
		goto out;
	}
...
rfc_print_rule(rfc_rule_st *rule)
{
	...
	printk("max_rule_num	%d\n", rule->max_rule_num);
	printk("cur_bitmap_size	 %d\n", rule->cur_bitmap_size);
	printk("fu8_num	%d\n", rule->fu8_num);
	printk("fu16_num	%d\n", rule->fu16_num);
