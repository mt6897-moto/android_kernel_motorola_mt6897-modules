	...
	unsigned int pkt_type, unsigned int rule_idx, unsigned int action, unsigned int uid)
{
	...
	*(unsigned int *)afw_pkt_log[num].dip = dip;
	...
	afw_pkt_log[num].pid = current->pid;
	memcpy(afw_pkt_log[num].comm, current->comm, sizeof(afw_pkt_log[num].comm));
...
{
	afw_rule_st *rule = NULL;
	unsigned char *p;
	int len;
	int offset;

	len = afw_rule_len(req);
	p = (unsigned char *)kmalloc(len, GFP_ATOMIC);
	if(!p) {
	...
	offset = sizeof(afw_rule_st);
	if(req->raw_rule) {
		rule->raw_rule = &p[offset];
		offset += AFW_ALIGN_4(req->raw_rule_len + 1);
	}

	if(req->domain) {
		rule->domain= &p[offset];
		offset += AFW_ALIGN_4(req->domain_len + 1);
	}

	if(req->port) {
		rule->port = (range_u16_st *)&p[offset];
	}
...
/* 1)清空黑名单规则，但是不包括app规则，即不包括uid规则 
 * 2)如果ipv4地址规则或者ipv6地址规则数为0，则不释放和重新初始化rfc，避免重新初始化rfc时申请不到内存
 * */
static void
afw_free_black_rule_ex(void)
{
	int ret;
	afw_rule_st *rule;
	afw_rule_st *next;
...
/* 1)清空白名单规则，但是不包括app规则，即不包括uid规则 
 * 2)如果ipv4地址规则或者ipv6地址规则数为0，则不释放和重新初始化rfc，避免重新初始化rfc时申请不到内存
 * */
static void
afw_free_white_rule_ex(void)
{
	int ret;
	afw_rule_st *rule;
	afw_rule_st *next;
...
int
afw_get_def_rule(afw_req_st *req)
{
	int ret;
...
/* 1)清空规则，但是不包括app规则，即不包括uid规则 
 * 2)不修改缺省规则
 * */
int
afw_flush_rule_ex(afw_req_st *req)
{
	int ret;
	...
	write_lock_bh(&afw_lock);
	if(req->type == AFW_RULE_TYPE_BLACK || req->type == AFW_RULE_TYPE_ALL) {
		afw_free_black_rule_ex();
	}

	if(req->type == AFW_RULE_TYPE_WHITE || req->type == AFW_RULE_TYPE_ALL) {
		afw_free_white_rule_ex();
	}
...
afw_test_rule_ipv4(int type)
{
	...
	unsigned long long normal_match_cost;
	unsigned long long fast_match_cost;
	unsigned long long slow_match_cost;

	int normal_match_num;
	int fast_match_num;
	int slow_match_num;
	int flag;
	...
	for(i = 0; i < TEST_NUM; i ++) {
		...
		normal_match_num += flag;
	}
	end = get_cycles();
	normal_match_cost = end - start;
	...
	printk("normal_match_num:	%d\n", normal_match_num);
	printk("fast_match_num:		%d\n", fast_match_num);
	printk("slow_match_num:		%d\n", slow_match_num);
	printk("\n");

	printk("normal_match_cost:	%lld\n", normal_match_cost);
	printk("fast_match_cost:		%lld\n", fast_match_cost);
	printk("slow_match_cost:		%lld\n", slow_match_cost);
	printk("\n");
	...
	unsigned int pkt_type, unsigned int uid)
{
	...
	#ifdef AFW_CUSTOM_KOREA
	if(afw_conf.pass_dns) {
		if(dport == DNS_PORT || dport == DNS_OVER_TLS_PORT) {
...
afw_rule_init(void)
{
	...
	#ifdef AFW_CUSTOM_XUNFEI
	def_host_rule = AFW_RULE_TYPE_BLACK;
	#else
	def_host_rule = AFW_RULE_TYPE_WHITE;
	#endif
