...
static int swtp_switch_state(int irq, struct swtp_t *swtp)
{
	...
	if((swtp->gpio_state[0] == SWTP_EINT_PIN_PLUG_OUT)&&(swtp->gpio_state[1] == SWTP_EINT_PIN_PLUG_OUT))
	{
		swtp->tx_power_mode = SWTP_DO_TX_POWER;
	CCCI_LEGACY_ERR_LOG(-1, SYS,
			 "%s>>tx_power_mode = SWTP_DO_TX_POWER ANT5 = %d ANT6 = %d\n" ,__func__,swtp->gpio_state[0],swtp->gpio_state[1]);
	}
	else
	{
		swtp->tx_power_mode = SWTP_NO_TX_POWER;
	CCCI_LEGACY_ERR_LOG(-1, SYS,
			 "%s>>tx_power_mode = SWTP_NO_TX_POWER ANT5 = %d ANT6 = %d\n" ,__func__,swtp->gpio_state[0],swtp->gpio_state[1]);
	}
