...
#define TYPEC_OTG_SYSTEM_BOOT_RETRY_MAX	5
#define TYPEC_OTG_ENABLE_RETRY_MAX	15
static int mtk_usb_typec_mux_otg_enable(struct mtk_extcon_info *extcon, bool enable)
{
	...
	do {
		chg_psy = power_supply_get_by_name("mtk-mst-div-chg");
		if (IS_ERR_OR_NULL(chg_psy)) {
			dev_err(dev, "typec_mux_otg: fail to get chg_psy\n");
		} else {
			info = (struct mtk_charger *)power_supply_get_drvdata(chg_psy);
			if (IS_ERR_OR_NULL(chg_psy)) {
				dev_err(dev, "typec_mux_otg: fail to get charger drvdate\n");
			} else {
				break;
			}
		}
		...
		retry++;
		msleep(1000);
	} while (retry < TYPEC_OTG_SYSTEM_BOOT_RETRY_MAX);

	retry = 0;
	do {
		ret = charger_dev_typec_mux_otg_enable(info->dvchg1_dev, enable);
		if (ret == (-EOPNOTSUPP)) {
			dev_err(dev, "typec_mux_otg: fail to enable: %d\n",  retry);
			retry++;
			msleep(1000);
		} else {
			return 0;
		}
	} while (retry < TYPEC_OTG_ENABLE_RETRY_MAX);
							...
							bool is_on)
{
	...
	if (is_on) {
		//+PERIDOT-583, yangpingao.wt, 20240413, mod, enable ovp mos by dvchg
		mtk_usb_typec_mux_otg_enable(extcon, true);
		//-PERIDOT-583, yangpingao.wt, 20240413, mod, enable ovp mos by dvchg
		...
		//+PERIDOT-583, yangpingao.wt, 20240413, mod, enable ovp mos by dvchg
		mtk_usb_typec_mux_otg_enable(extcon, false);
		//-PERIDOT-583, yangpingao.wt, 20240413, mod, enable ovp mos by dvchg
	}
	...
	case TCP_NOTIFY_DR_SWAP:
		...
		if (noti->swap_state.new_role == PD_ROLE_UFP &&
           extcon->c_role != USB_ROLE_DEVICE) {
               dev_info (dev, "%s switch role to device\n", __func__);
               /*swch_d_time = jiffies;
               if (time_after(swch_h_time + 10*HZ, swch_d_time)) {
                    pr_err("%s delay for device switch\n", __func__);
                    msleep(500);
               }
               else{
			    ...
			    */
			    //msleep(30);
			   //dev_info(dev,"%s delay 30ms swtich role to device!\n",__func__);
               mtk_usb_extcon_set_role(extcon, USB_ROLE_DEVICE);
        } else if (noti->swap_state.new_role == PD_ROLE_DFP && 
            extcon->c_role != USB_ROLE_HOST) {
               dev_info (dev, "%s switch role to host\n", __func__);
               /*swch_h_time = jiffies;
               if (time_after(swch_d_time + 10*HZ, swch_h_time)) {
                   pr_err("%s delay for host switch\n", __func__);
                   msleep(500);
               }
               else{
         	...
         	dev_info(dev,"Delay for swap...(500ms)\n");
			msleep(500);
         	goto DelayforSwap;
			}
		break;
