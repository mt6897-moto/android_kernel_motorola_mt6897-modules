...
//char Ctp_name[HARDWARE_MAX_ITEM_LONGTH];// wanglongfei.wt 20200629 add for tp hardware info
char hallname[HARDWARE_MAX_ITEM_LONGTH];
//char Lcm_name[HARDWARE_MAX_ITEM_LONGTH];//req  wuzhenzhen.wt 20140901 add for hardware info //checklist-98164,zhangshaoxiong1.wt,modify,20240221,add lcd hardware info
...
static long hardwareinfo_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	case HARDWARE_LCD_GET:
		//hardwareinfo_set_prop(HARDWARE_LCD, Lcm_name);//req  wuzhenzhen.wt 20140901 add for hardware info //checklist-98164,zhangshaoxiong1.wt,modify,20240221,add lcd hardware info
		hardwareinfo_num = HARDWARE_LCD;
		break;
	case HARDWARE_TP_GET:
		//hardwareinfo_set_prop(HARDWARE_TP, Ctp_name);
		hardwareinfo_num = HARDWARE_TP;
		//tid_hardware_info_get(hardwareinfo_name[hardwareinfo_num],ARRAY_SIZE(hardwareinfo_name[0]));
		break;
        
//+PERIDOT-494  yizefeng.wt add 20240305 add hall sensor name
       case HARDWARE_HALL_GET:
		hardwareinfo_num = HARDWARE_HALL;
		break;
       case HARDWARE_HALL2_GET:
		hardwareinfo_set_prop(HARDWARE_HALL2, "OCH166ASTAD");//req  wuzhenzhen.wt 20140901 add for hardware info
		hardwareinfo_num = HARDWARE_HALL2;
		break;
//+PERIDOT-494  yizefeng.wt add 20240305 add hall sensor name
	...
	case HARDWARE_BT_GET:
		hardwareinfo_set_prop(HARDWARE_BT, "MT6637X");
		hardwareinfo_num = HARDWARE_BT;
		break;
	case HARDWARE_WIFI_GET:
		hardwareinfo_set_prop(HARDWARE_WIFI, "MT6637X");
	...
	case HARDWARE_GPS_GET:
		hardwareinfo_set_prop(HARDWARE_GPS, "MT6637X");
	    hardwareinfo_num = HARDWARE_GPS;
		break;
	case HARDWARE_FM_GET:
		hardwareinfo_set_prop(HARDWARE_FM, "MT6637X");
	...
	case HARDWARE_CHARGER_IC_SLAVE_INFO_GET:
  		hardwareinfo_num = HARDWARE_CHARGER_IC_SLAVE_INFO;
 		break;
	//-Peridot-35,xiaohongyu,wt,add,20240223,add CHARGE IC message in factory mode
	...
	case HARDWARE_SMARTPA_IC_GET:
		hardwareinfo_set_prop(HARDWARE_SMART_PA_ID, "AW88394");
...
static ssize_t show_boardinfo(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for(i = 0; i < HARDWARE_MAX_ITEM; i++)
	{
		...
		//+checklist-98164,zhangshaoxiong1.wt,modify,20240221,add lcd hardware info
		/*if(i == HARDWARE_LCD)
		{
