...
struct ps5170 {
	...
	struct power_supply *ps5170_psy;
};
...
static int ps5170_init(struct ps5170 *ps)
{
	...
	ret = i2c_smbus_read_word_data(ps->i2c, 0x52);
	if (ret == 0x20) {
		is_exits_ps5170 = 1;
	} else {
		is_exits_ps5170 = 2;
...
//+Peridot-35, liyiying.wt, 20240612, add, add del usb redriver
static enum power_supply_property ps5170_battery_props[] = {
	POWER_SUPPLY_PROP_TEMP,
};

static int ps5170_get_property(struct power_supply *psy,
...
static int ps5170_probe(struct i2c_client *client)
{
	...
	psy_cfg.drv_data = ps;
	psy_desc->name = "PS5170";
	psy_desc->type = POWER_SUPPLY_TYPE_UNKNOWN;
	psy_desc->properties = ps5170_battery_props;
	psy_desc->num_properties = ARRAY_SIZE(ps5170_battery_props);
	psy_desc->get_property = ps5170_get_property;
	psy_desc->external_power_changed = NULL;
	psy_desc->set_property = NULL;
	psy_desc->property_is_writeable = NULL;

	ps->ps5170_psy = power_supply_register(&client->dev, psy_desc, &psy_cfg);
	if (IS_ERR(ps->ps5170_psy)) {
		ret = PTR_ERR(ps->ps5170_psy);
		pr_info("failed to register ps5170: %d\n", ret);
