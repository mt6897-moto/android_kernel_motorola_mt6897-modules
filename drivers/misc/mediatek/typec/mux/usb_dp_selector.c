...
//+PERIDOT-35, 20240222, liyiying.wt, add, add dp function
#if IS_ENABLED(CONFIG_FSA4480_I2C_DF)
static enum typec_orientation cc_orientation;
#endif
//-PERIDOT-35, 20240222, liyiying.wt, add, add dp function
			      ...
			      enum typec_orientation orientation)
{
	...
	dev_info(uds->dev, "%s %d cc_add:%d\n", __func__, orientation, cc_orientation);
	#else
...
#if IS_ENABLED(CONFIG_FSA4480_I2C_DF)
		fsa4480_switch_event(FSA_USBC_ORIENTATION_CC1);
#endif
...
#if IS_ENABLED(CONFIG_FSA4480_I2C_DF)
		fsa4480_switch_event(FSA_USBC_ORIENTATION_CC2);
#endif
		/* USB FLIP TX2, DP TX1 */
		switch (uds->uds_ver) {
		case uds_V1:
				...
				struct typec_mux_state *state)
{
	...
	if (dp_data->conf) {
		...
		//+PERIDOT-5421,20240516,xiaohongyu,wt,add,fix the screen projector disconnect problem
