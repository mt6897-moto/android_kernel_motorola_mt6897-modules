...
#include <linux/of_gpio.h>//PERIDOT-422 liaotingying.wt,modify,20240222, add sdcard slot node
				   ...
				   struct msdc_host *host)
{
	...
	of_property_read_u32(pdev->dev.of_node, "mediatek,hs400-cmd-int-delay",
			     &host->hs400_cmd_int_delay);
	cd_gpio=of_get_named_gpio(pdev->dev.of_node, "cd-gpios", 0);
...
//PERIDOT-422 liaotingying.wt,modify,20240222, add sdcard slot node end
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if(sim_card_tray_create_proc())//PERIDOT-422 liaotingying.wt,modify,20240222, add sdcard slot node
		dev_err(&pdev->dev, "creat proc sim_card status failed\n");
...
host_free:
	...
	if(!strcmp(mmc_hostname(mmc), "mmc1"))
		sim_card_tray_remove_proc();//PERIDOT-422 liaotingying.wt,modify,20240222, add sdcard slot node
