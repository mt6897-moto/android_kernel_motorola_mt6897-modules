...
static void read_tx_lctxc0_tx_lctxcp1_value(struct mtk_xsphy *xsphy,
			     struct xsphy_instance *inst)
{
	void __iomem *pbase = inst->port_base;
	u32 tmp1, tmp2;

	tmp1 = readl(pbase + SSPXTP_DAIG_LN_DAIF_00);
	pr_info("%s tmp is %d\n", __func__, tmp1);
	tmp1 &= RG_XTP0_DAIF_FRC_LN_TX_LCTXC0;
	pr_info("%s tmp is %d\n", __func__, tmp1);

	tmp1 = readl(pbase + SSPXTP_DAIG_LN_DAIF_08);
	pr_info("%s tmp is %d\n", __func__, tmp1);
	tmp1 >>= RG_XTP0_DAIF_LN_TX_LCTXC0_OFST;
	pr_info("%s tmp is %d\n", __func__, tmp1);
	tmp1 &= RG_XTP0_DAIF_LN_TX_LCTXC0_MASK;
	pr_info("%s tmp is %d\n", __func__, tmp1);

	tmp2 = readl(pbase + SSPXTP_DAIG_LN_DAIF_00);
	pr_info("%s tmp is %d\n", __func__, tmp1);
	tmp2 &= RG_XTP0_DAIF_FRC_LN_TX_LCTXCP1;
	pr_info("%s tmp is %d\n", __func__, tmp1);

	tmp2 = readl(pbase + SSPXTP_DAIG_LN_DAIF_08);
	pr_info("%s tmp is %d\n", __func__, tmp1);
	tmp2 >>= RG_XTP0_DAIF_LN_TX_LCTXCP1_OFST;
	pr_info("%s tmp is %d\n", __func__, tmp1);
	tmp2 &= RG_XTP0_DAIF_LN_TX_LCTXCP1_MASK;
	pr_info("%s tmp is %d\n", __func__, tmp1);
}

static u32 ori_tx_lctxc0, ori_tx_lctxcp1;
static bool run_one_times = false;
static void save_tx_lctxc0_tx_lctxcp1_value(struct mtk_xsphy *xsphy,
			     struct xsphy_instance *inst)
{
	void __iomem *pbase = inst->port_base;
	u32 tmp1, tmp2;
...
static void clear_tx_lctxc0_tx_lctxcp1_value(struct mtk_xsphy *xsphy,
			     struct xsphy_instance *inst)
{
	void __iomem *pbase = inst->port_base;

	//clear tx_lctxc0
	mtk_phy_clear_bits(pbase + SSPXTP_DAIG_LN_DAIF_00, RG_XTP0_DAIF_FRC_LN_TX_LCTXC0);
	mtk_phy_update_field(pbase + SSPXTP_DAIG_LN_DAIF_08, RG_XTP0_DAIF_LN_TX_LCTXC0, ori_tx_lctxc0);

	//clear tx_lctxcp1
	mtk_phy_clear_bits(pbase + SSPXTP_DAIG_LN_DAIF_00, RG_XTP0_DAIF_FRC_LN_TX_LCTXCP1);
	mtk_phy_update_field(pbase + SSPXTP_DAIG_LN_DAIF_08, RG_XTP0_DAIF_LN_TX_LCTXCP1, ori_tx_lctxcp1);
}

static bool ps5170_device_exits;
static int device_exits(struct mtk_xsphy *xsphy)
{
	...
	if (psy == NULL || IS_ERR(psy)) {
		...
		is_exits = val.intval;
		pr_err("%s Can get psy, val is %d %d\n", __func__, is_exits, val.intval);
	}
...
static bool is_exit_ps5170(struct mtk_xsphy *xsphy,
			     struct xsphy_instance *inst)
{
	if (device_exits(xsphy) ||ps5170_device_exits == true) {
		inst->tx_lctxc0 = -EINVAL;
		inst->tx_lctxcp1 = -EINVAL;

		clear_tx_lctxc0_tx_lctxcp1_value(xsphy, inst);
		read_tx_lctxc0_tx_lctxcp1_value(xsphy, inst);

		dev_info(xsphy->dev, "ps5170 is exits\n");
		return true;
	} else {
		inst->tx_lctxc0 = 34;
		inst->tx_lctxcp1 = 14;

		dev_info(xsphy->dev, "ps5170 is not exits\n");
		return false;
	}
}
//-Peridot-35, liyiying.wt, 20240612, add, add del usb redriver
			     ...
			     struct xsphy_instance *inst)
{
	...
	if (run_one_times == false) {
		save_tx_lctxc0_tx_lctxcp1_value(xsphy, inst);
		run_one_times = true;
	}
	is_exit_ps5170(xsphy, inst);
	//-Peridot-35, liyiying.wt, 20240612, add, add del usb redriver
