...
//+PERIDOT-583, yangpingao.wt, 20240413, mod, enable ovp mos by dvchg
int charger_dev_typec_mux_otg_enable(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL && chg_dev->ops->typec_mux_otg_enable)
		return chg_dev->ops->typec_mux_otg_enable(chg_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_typec_mux_otg_enable);
//-PERIDOT-583 yangpingao.wt, 20240413, mod, enable ovp mos by dvchg

//+PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic
int charger_dev_reset_qc20_ta(struct charger_device *chg_dev)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->reset_qc20_ta)
		return chg_dev->ops->reset_qc20_ta(chg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_reset_qc20_ta);

int charger_dev_set_fast_charging_status(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_fast_charging_status)
		return chg_dev->ops->set_fast_charging_status(chg_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_fast_charging_status);
//-PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic

//+PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp
int charger_dev_set_bat_ircomp(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_bat_ircomp)
		return chg_dev->ops->set_bat_ircomp(chg_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_bat_ircomp);
//-PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp
...
int charger_dev_set_shipping_mode(struct charger_device *chg_dev, u32 val)
{
	if (chg_dev != NULL && chg_dev->ops != NULL && chg_dev->ops->set_shipping)
		return chg_dev->ops->set_shipping(chg_dev, val);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_shipping_mode);
//-PERIDOT-203, liyiying.wt, mod, 20240313, add aidl shipmode interface
