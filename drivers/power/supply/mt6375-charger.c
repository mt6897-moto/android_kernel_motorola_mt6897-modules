...
#define MT6375_REG_DPDM_CTRL3	0x155 //+PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic
#define MT6375_REG_DPDM_CTRL4	0x156
#define MT6375_REG_DPDM_CTRL5	0x157	//+PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic
...
#define DPDM_REG_MAX	5	//+PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic

//+Peridot-35,xiaohongyu,wt,add,20240223,add charge ic message in factory mode
#define CHARGER_IC_NAME "MT6375"
//-Peridot-35,xiaohongyu,wt,add,20240223,add charge ic message in factory mode

//+PERIDOT-203, liyiying.wt, mod, 20240313, add aidl shipmode interface
static int _mt6375_set_shipping_mode(struct charger_device *chgdev, u32 val);
//-PERIDOT-203, liyiying.wt, mod, 20240313, add aidl shipmode interface
...
struct mt6375_chg_data {
	...
	u32 bc12_real_type;
	u32 dpdm_reg_val[DPDM_REG_MAX];
	bool is_pd30_support;
	//-PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic
};
...
{
	int ret;
	dev_err(ddata->dev, "mt6375_chg_enable_charging: =%d\n", en);
...
//+PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic
#define HVDCP_QC20_DM_CHECK_COUNT 25
#define HVDCP_QC20_DM_VOLT_MAX	400
#define HVDCP_QC20_VBUS_CHECK_COUNT 10
#define HVDCP_QC20_VBUS_VOLT_MIN	7500

static int mt6375_chg_qc20_detect(struct mt6375_chg_data *ddata)
{
	int i, ret = 0;
	int dm_volt, vbus_volt;
	int dpdm_reg;
	struct chgdev_notify *mtk_chg_noti = &(ddata->chgdev->noti);

	msleep(50);
	if (ddata->is_pd30_support == true) {
		dev_err(ddata->dev, "PD3.0 charger detect success, skip hvdcp detect\n");
		return ret;
	}

	mtk_chg_noti->qc20_vbus_ovp_en = false;
	charger_dev_notify(ddata->chgdev,
					   CHARGER_DEV_NOTIFY_QC20_DETECT);
	dpdm_reg = MT6375_REG_DPDM_CTRL1;
	for (i = 0; i < DPDM_REG_MAX; i++) {
		ret = regmap_read(ddata->rmap, dpdm_reg, &ddata->dpdm_reg_val[i]);
		if (ret < 0) {
			dev_err(ddata->dev, "failed to read reg: 0x%x \n", dpdm_reg);
			return ret;
		}
		mt_dbg(ddata->dev, "dp_dm regs: 0x%x, val: 0x%x\n",
				dpdm_reg, ddata->dpdm_reg_val[i]);
		dpdm_reg += 1;
	}

	//1-- Set DP: 0.6V, DM: HIZ
	regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL1, 0xA3);
	regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL2, 0x80);
	regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL3, 0x02);
	regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL4, 0x0C);
	regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL5, 0x00);

	//2-- Wait HVDCP detect 1.3S, check dm voltage less chan 0.325V
	for (i = 0; i <= HVDCP_QC20_DM_CHECK_COUNT; i++) {
		msleep(50);
		ret = iio_read_channel_processed(&ddata->iio_adcs[ADC_CHAN_USBDM],
				&dm_volt);
		if (ret < 0) {
		...
		dm_volt = U_TO_M(dm_volt);
		mt_dbg(ddata->dev, "hvdcp detect: check count: %d, dm_volt: %d\n",
				i, dm_volt);
		if (dm_volt <= HVDCP_QC20_DM_VOLT_MAX) {
			dev_err(ddata->dev, "hvdcp dm check: success !\n");
			break;
		}

		if (i >= HVDCP_QC20_DM_CHECK_COUNT) {
			dev_err(ddata->dev, "hvdcp dm check time out !: %d\n", i);
			goto HVDCP_DETECT_EXIT;
		}
	}

	//3-- Request QC20 9V mode, set DP: 3.3V, DM: 0.6V
	msleep(150);
	if (ddata->is_pd30_support == true) {
		dev_err(ddata->dev, "PD3.0 charger detect success, skip hvdcp detect\n");
		return ret;
	}
	regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL2, 0xE8);
	regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL3, 0x00);
	regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL4, 0xC0);
	regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL5, 0x0C);

	//4-- Wait HVDCP detect 0.5S, check Vbus greater chan 7500mV
	for (i = 0; i <= HVDCP_QC20_VBUS_CHECK_COUNT; i++) {
		msleep(50);
		ret = iio_read_channel_processed(&ddata->iio_adcs[ADC_CHAN_CHGVINDIV5],
				&vbus_volt);
		if (ret < 0) {
			dev_err(ddata->dev, "failed to read vbus \n");
			continue;
		}

		vbus_volt = U_TO_M(vbus_volt);
		dev_info(ddata->dev, "hvdcp detect :check count: %d, volt: %d\n",
				i, vbus_volt);
		if (vbus_volt >= HVDCP_QC20_VBUS_VOLT_MIN) {
			dev_info(ddata->dev, "hvdcp detect: request 9v mode success !\n");
			ddata->bc12_real_type = POWER_SUPPLY_BC12_TYPE_QC20;
			charger_dev_notify(ddata->chgdev,
					   CHARGER_DEV_NOTIFY_QC20_DETECT);
			return 0;
		}
	...
	//5-- QC2.0 detect fail, force 5V mode, set DP: 0.6V, DM: 0V
HVDCP_DETECT_EXIT:
	dpdm_reg = MT6375_REG_DPDM_CTRL1;
	for (i = 0; i < DPDM_REG_MAX; i++ ) {
		ret = regmap_write(ddata->rmap, dpdm_reg, ddata->dpdm_reg_val[i]);
		if (ret < 0) {
			dev_err(ddata->dev, "failed to write reg: 0x%x \n", dpdm_reg);
		}
	...
	mtk_chg_noti->qc20_vbus_ovp_en = true;
	charger_dev_notify(ddata->chgdev,
					   CHARGER_DEV_NOTIFY_QC20_DETECT);
	return ret;
}

static int mt6375_reset_qc20_ta(struct charger_device *chgdev)
{
	int i, ret;
	int dpdm_reg;
	...
	ddata->is_pd30_support = false;
	//1-- QC2.0 force 5V mode, set DP: 0.6V, DM: 0V
	if (ddata->bc12_real_type == POWER_SUPPLY_BC12_TYPE_QC20) {
		regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL1, 0xA3);
		regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL2, 0x80);
		regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL3, 0x00);
		regmap_write(ddata->rmap, MT6375_REG_DPDM_CTRL4, 0x0C);
	...
	//2-- DP_DM register restore
	dpdm_reg = MT6375_REG_DPDM_CTRL1;
	for (i = 0; i < DPDM_REG_MAX; i++ ) {
		ddata->dpdm_reg_val[i] = 0;
		ret = regmap_write(ddata->rmap, dpdm_reg, ddata->dpdm_reg_val[i]);
		if (ret < 0) {
			dev_err(ddata->dev, "failed to write reg: 0x%x \n", dpdm_reg);
		}
...
static int mt6375_set_fast_charging_status(struct charger_device *chgdev, bool en)
{
	struct mt6375_chg_data *ddata = charger_get_data(chgdev);

	ddata->is_pd30_support = en;
...
out_no_bc12_ctrl:
	...
	switch (val) {
	case PORT_STAT_APPLE_5W:
		ddata->bc12_real_type = POWER_SUPPLY_BC12_TYPE_APPLE_5W;
		break;
	case PORT_STAT_APPLE_10W:
		ddata->bc12_real_type = POWER_SUPPLY_BC12_TYPE_APPLE_10W;
		break;
	case PORT_STAT_APPLE_12W:
		ddata->bc12_real_type = POWER_SUPPLY_BC12_TYPE_APPLE_12W;
		break;
	case PORT_STAT_SAMSUNG:
		ddata->bc12_real_type = POWER_SUPPLY_BC12_TYPE_SAMSUNG;
		mt6375_chg_qc20_detect(ddata);
		break;
	case PORT_STAT_DCP:
		ddata->bc12_real_type = POWER_SUPPLY_BC12_TYPE_DCP;
		mt6375_chg_qc20_detect(ddata);
		break;
	case PORT_STAT_SDP:
		ddata->bc12_real_type = POWER_SUPPLY_BC12_TYPE_SDP;
		break;
	case PORT_STAT_CDP:
		ddata->bc12_real_type = POWER_SUPPLY_BC12_TYPE_CDP;
		break;
	default:
		ddata->bc12_real_type = POWER_SUPPLY_BC12_TYPE_UNKNOWN_TA;
	}
	//-PERIDOT-35, yangpingao.wt, 20240314, mod, qc2.0 detect logic
}
...
static enum power_supply_property mt6375_chg_psy_properties[] = {
	...
	POWER_SUPPLY_PROP_CHARGE_TYPE,	//+PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		mutex_lock(&ddata->attach_lock);
		val->intval = ddata->bc12_real_type;
		mutex_unlock(&ddata->attach_lock);
		break;
	//-PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic
...
static int mt6375_set_bat_ircomp(struct charger_device *chgdev, bool en)
{
	int ret;
	int ircomp_reg;
	u32 ircomp_reg_val;
	...
	ircomp_reg = MT6375_REG_BAT_COMP;
	ret = regmap_read(ddata->rmap, ircomp_reg, &ircomp_reg_val);
	if (ret < 0) {
		dev_err(ddata->dev, "failed to read reg: 0x%x \n", ircomp_reg);
	}
	mt_dbg(ddata->dev, "read regs: 0x%x, val: 0x%x\n",
			ircomp_reg, ircomp_reg_val);
	...
	mt_dbg(ddata->dev, "write en: %d, val: 0x%x\n",
			en, ircomp_reg_val);
	ret = regmap_write(ddata->rmap, ircomp_reg, ircomp_reg_val);
	if (ret < 0) {
		dev_err(ddata->dev, "failed to read reg: 0x%x \n", ircomp_reg);
	}
...
static int mt6375_sw_check_eoc(struct charger_device *chgdev, u32 uA)
{
	...
	dev_err(ddata->dev, "ieoc=%d, ibat=%d, eoc_cnt=%d\n", uA, ibat, atomic_read(&ddata->eoc_cnt));
	if (ibat <= uA) {
		/* if it happens 3 times, trigger EOC event */
		if (atomic_read(&ddata->eoc_cnt) == 3) {
...
static const struct charger_ops mt6375_chg_ops = {
	...
	//+PERIDOT-203, liyiying.wt, mod, 20240313, add aidl shipmode interface
	.set_shipping = _mt6375_set_shipping_mode,
	//-PERIDOT-203, liyiying.wt, mod, 20240313, add aidl shipmode interface

	//+PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic
	/* QC20 */
	.reset_qc20_ta = mt6375_reset_qc20_ta,
	.set_fast_charging_status = mt6375_set_fast_charging_status,
	//-PERIDOT-2894, yangpingao.wt, 20240423, mod, qc2.0 detect logic

	//+PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp
	.set_bat_ircomp = mt6375_set_bat_ircomp,
	//-PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp
};
...
static int mt6375_set_shipping_mode_delay_18s(struct mt6375_chg_data *ddata)
{
	int ret;
...
static int _mt6375_set_shipping_mode(struct charger_device *chgdev, u32 val)
{
	...
	mt_dbg(ddata->dev, "setting value = %d\n", val);

	if (val != 5526789)
		return -EINVAL;

	ret = mt6375_set_shipping_mode_delay_18s(ddata);
...
static int mt6375_chg_probe(struct platform_device *pdev)
{
	...
	//+Peridot-35,xiaohongyu,wt,add,20240223,add charge ic message in factory mode
	hardwareinfo_set_prop(HARDWARE_CHARGER_IC_INFO, CHARGER_IC_NAME);
	//-Peridot-35,xiaohongyu,wt,add,20240223,add charge ic message in factory mode
