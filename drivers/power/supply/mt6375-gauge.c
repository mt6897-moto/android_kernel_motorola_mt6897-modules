			   ...
			   enum mt6375_gauge_cic_idx cic_idx)
{
	...
	union power_supply_propval ext_curr;
	#endif
	...
	if (ext_psy == NULL) {
		bm_err("[%s] mm8013 psy is not ready \n", __func__);
		//+PERIDOT-549, liyiying.wt, add, 20240313, PCBA testing shows abnormal battery voltage
		*val = dvalue;
		//-PERIDOT-549, liyiying.wt, add, 20240313, PCBA testing shows abnormal battery voltage
	} else {
		ret = power_supply_get_property(ext_psy, POWER_SUPPLY_PROP_CURRENT_NOW, &ext_curr);
		if (ret < 0) {
			bm_err("[%s] mm8013 get curr_now fail\n", __func__);
			//+PERIDOT-549, liyiying.wt, add, 20240313, PCBA testing shows abnormal battery voltage
			*val = dvalue;
			//-PERIDOT-549, liyiying.wt, add, 20240313, PCBA testing shows abnormal battery voltage
		} else {
	...
	//pr_notice("[wt-debug] - %s mm8013_curr_now is: %d \n", __func__, ext_curr.intval);
	#else
	*val = dvalue;
	#endif
	//-PERIDOT-35, liyiying.wt, add, 20240204, mm8013 bring up
	...
	*data = gauge_dev->fg_hw_info.current_avg;
	...
	if (ext_psy == NULL) {
		...
		*data = gauge_dev->fg_hw_info.current_avg;
		...
		ret = power_supply_get_property(ext_psy, POWER_SUPPLY_PROP_CURRENT_AVG, &ext_curr);
		if (ret < 0) {
			bm_err("[%s] mm8013 get curr_avg fail\n", __func__);
		} else {
			*data = ext_curr.intval;
			iavg_vld = 1;
		}
	}
	bm_debug("[wt-debug] - %s mm8013_curr_avg is: %d \n", __func__, ext_curr.intval);
	#endif
	//-PERIDOT-35, liyiying.wt, add, 20240204, mm8013 bring up
		    ...
		    int *zcv_current)
{
	...
	*zcv_current = dvalue;
	...
	if (ext_psy == NULL) {
		bm_err("[%s] mm8013 psy is not ready --3\n", __func__);
		//+PERIDOT-549, liyiying.wt, add, 20240313, PCBA testing shows abnormal battery voltage
		*zcv_current = dvalue;
		//-PERIDOT-549, liyiying.wt, add, 20240313, PCBA testing shows abnormal battery voltage
	} else {
		...
		if (ret < 0) {
			...
			*zcv_current = ext_curr.intval;
		}
	}
	bm_debug("[wt-debug] - %s mm8013_curr_now is: %d \n", __func__, ext_curr.intval);
	#endif
	//-PERIDOT-35, liyiying.wt, add, 20240204, mm8013 bring up
...
{
	struct power_supply *ext_psy;
	union power_supply_propval ext_vol;
	struct mt6375_priv *priv = container_of(gauge, struct mt6375_priv, gauge);
	...
	if (ext_psy == NULL) {
		...
		ret = power_supply_get_property(ext_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &ext_vol);
		if (ret < 0) {
			bm_err("[%s] mm8013 get vol fail\n", __func__);
		} else {
			*val = ext_vol.intval;
			bm_debug("[wt-debug] - %s mm8013_vol is: %d \n", __func__, ext_vol.intval);
			return ret;
		}
	}
