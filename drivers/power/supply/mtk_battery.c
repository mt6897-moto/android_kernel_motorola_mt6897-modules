	...
	union power_supply_propval *val)
{
	...
	//+PERIDOT-941, liyiying.wt, mod, 20240415, Repeatedly plug and unplug the charger, which is an unreasonable estimation
	chg_master_psy = power_supply_get_by_name("primary_chg");
	//-PERIDOT-941, liyiying.wt, mod, 20240415, Repeatedly plug and unplug the charger, which is an unreasonable estimation

	//+PERIDOT-4482, liyiying.wt, 20240413, mod, fluctuation in battery level
	/*
	...
	*/
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		//+PERIDOT-35, liyiying.wt, add, 20240204, mm8013 bring up
	...
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		...
		//+PERIDOT-35, liyiying.wt, add, 20240304, When the ATO version is low on power, inserting a charger will cause repeated restarts and prevent booting up
		if (bs_data->bat_capacity == 0) {
			if (bs_data->bat_batt_vol > SHUTDOWN_VOLTAGE) {
	...
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		...
		if ((ret == POWER_SUPPLY_CAPACITY_LEVEL_FULL) ||
			...
			if (current_now != 0)
				...
				bm_debug("time_to_full:%d, remain:ui:%d mah:%d, current_now:%d, qmax:%d, chg_type:%d\n",
					time_to_full, remain_ui, remain_mah,
					current_now, q_max_now, propval.intval);
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		if (check_cap_level(bs_data->bat_capacity) ==
			...
			val->intval = 10200000;    //+peridot-1342,xiaohongyu,wt,add,20240328,modify the full charge design
		}
		break;
...
void battery_service_data_init(struct mtk_battery *gm)
{
	...
	bs_data->ext_fauge_bat_capacity = -1,
...
int adc_battemp(struct mtk_battery *gm, int res)
{
	...
	if (res >= ptable[0].TemperatureR) {
		tbatt_value = -40;
	//+Peridot-422, liyiying.wt, mod, 20240221, mod battery shutdown temperature
	} else if (res <= ptable[23].TemperatureR) {
		tbatt_value = 75;
	//-Peridot-422, liyiying.wt, mod, 20240221, mod battery shutdown temperature
	} else {
...
void fg_custom_init_from_header(struct mtk_battery *gm)
{
	...
	//+PERIDOT-35, liyiying.wt, add, 20240204, natt ntc bring up
	...
	struct mtk_battery *gm)
{
	...
	bm_err("is_evb_board is %d before\n", is_evb_board);
	is_evb_board = 0;
	bm_err("is_evb_board is %d after\n", is_evb_board);
	//-PERIDOT-422, liyiying.wt, 20240226, add, add ATO version battery  management (soc) control logic
...
//+PERIDOT-4482, liyiying.wt, 20240413, mod, fluctuation in battery level
static void battery_update_mm8013_soc(struct mtk_battery *gm)
{
	...
	if (ext_psy == NULL) {
		bm_err("[%s] mm8013 psy is not ready \n", __func__);
		if (bat_data->ext_fauge_bat_capacity != -1) {
			bat_data->bat_capacity = bat_data->ext_fauge_bat_capacity;
			gm->ui_soc = bat_data->ext_fauge_bat_capacity;
		} else {
		...
		if (ret < 0) {
			...
			if (ext_soc.intval >= 0) {
				bat_data->bat_capacity = ext_soc.intval;
				gm->ui_soc = ext_soc.intval;

				bat_data->ext_fauge_bat_capacity = ext_soc.intval;
				gm->ext_fauge_uisoc = ext_soc.intval;
			}
		}
	}
	//pr_err("[wt-debug] - %s mtk_cap is: %d, mm8013_cap is: %d \n", __func__, bat_data->bat_capacity, ext_soc.intval);
}
//-PERIDOT-4482, liyiying.wt, 20240413, mod, fluctuation in battery level
