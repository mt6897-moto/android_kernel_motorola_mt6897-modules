...
int get_battery_temperature(struct mtk_charger *info)
{
	...
	if (info->user_set_batt_temp_flag == true) {
		ret = info->user_set_batt_temp;
	}
	//-PERIDOT-35, yangpingao.wt, 20240314, mod, adjust battery jeita logic
...
int get_charger_real_type(struct mtk_charger *info)
{
	union power_supply_propval prop = {0};
	union power_supply_propval prop2 = {0};
	static struct power_supply *bc12_psy;
	...
	if (bc12_psy == NULL || IS_ERR(bc12_psy)) {
		...
		ret = power_supply_get_property(bc12_psy,
			POWER_SUPPLY_PROP_CHARGE_TYPE, &prop2);

		if (prop.intval == 0) {
			prop2.intval = POWER_SUPPLY_BC12_TYPE_UNKNOWN_TA;
		}
	}

	chr_debug("%s online:%d real_type:%d\n", __func__, prop.intval, prop2.intval);
