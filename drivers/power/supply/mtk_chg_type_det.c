...
extern void store_cc_orientation(int cc_ori);
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		dev_info(mci->dev, "%s TCP_NOTIFY_TYPEC_STATE pol = %d", __func__, noti->typec_state.polarity);
		if (noti->typec_state.new_state == TYPEC_UNATTACHED ||
						noti->typec_state.new_state == TYPEC_ATTACHED_NORP_SRC) {
			cc_value = 0;
		} else {
			cc_value = 1 + noti->typec_state.polarity;
		}
		store_cc_orientation(cc_value);
		//-PERIDOT-422, liyiying.wt, 20240224, add, add sys/class/wt-battery/typec_cc_orientation node
		break;
