...
static int _pd_is_algo_ready(struct chg_alg_device *alg)
{
	...
	if (algo_waiver_test) {
        ...
        pd_get_usb_type(&usb_type);
	pd_err("%s %d %d\n", __func__, pd->state, usb_type);

	//+PERIDOT-35, liyiying.wt, 20240323, mod, support PE2 and PD fast Charging
	if (usb_type == POWER_SUPPLY_TYPE_USB || usb_type == POWER_SUPPLY_TYPE_USB_CDP){
		...
		if (pd->charging_current_limit1 >= 3000000) {
			pd->charging_current_limit1 = -1;
		}
	}
        //pd->charging_current_limit1 = -1;
	...
	case PD_HW_READY:
		...
		if (ret_value == ALG_READY) {
			...
			if (pd->input_current_limit1 != -1 ||
				...
				pr_err("%s: %d %d %d %d -1", __func__, pd->input_current_limit1, pd->charging_current_limit1, pd->input_current_limit2, pd->charging_current_limit2);
			} else if ((uisoc >= pd->pd_stop_battery_soc && (usb_type != POWER_SUPPLY_TYPE_USB && usb_type != POWER_SUPPLY_TYPE_USB_CDP)) ||
			...
			int *newidx)
{
	...
	min_watt_value = cap->max_mv[pd->pd_buck_idx] * cap->ma[pd->pd_buck_idx]
			/ 100 * (100 - pd->ibus_err)
			- pd->vsys_watt;
        if (min_watt_value <= 0)
                pd_min_watt = 5000000;
        else
                pd_min_watt = min_watt_value;
	...
	//+Peridot-2175, liyiying.wt, add, 20240511, Tested the power of the prototype using Redmi K30 Ultra 33W charger, with a measured power of 8.38W
...
static int pd_dcs_set_charger(struct chg_alg_device *alg)
{
	...
	pr_err("%s m:%d s:%d cv:%d chg1:%d,%d chg2:%d,%d chg2en:%d min:%d,%d,%d\n",
		__func__,
		alg->config,
		pd->state,
		pd->cv,
...
static int _pd_start_algo(struct chg_alg_device *alg)
{
	...
	do {
                pd_get_usb_type(&usb_type);
		pr_err("%s state:%d %s %d usb_type :%d\n", __func__,
			pd->state,
		...
		case PD_HW_READY:
			...
			if (ret_value == ALG_TA_NOT_SUPPORT)
				...
				if (pd->input_current_limit1 != -1 ||
					...
					pd->charging_current_limit2 != -1) {
					ret_value = ALG_NOT_READY;
					pr_err("%s: %d %d %d %d -2", __func__, pd->input_current_limit1, pd->charging_current_limit1, pd->input_current_limit2, pd->charging_current_limit2);
				}
				else if ((uisoc >= pd->pd_stop_battery_soc && (usb_type != POWER_SUPPLY_TYPE_USB && usb_type != POWER_SUPPLY_TYPE_USB_CDP)) ||
                                          (uisoc == -1 && pd->ref_vbat > pd->vbat_threshold))
					ret_value = ALG_WAIVER;
...
int pd_get_usb_type(int *usb_type)
{
	...
	if (usb_psy == NULL)
		usb_psy = power_supply_get_by_name("primary_chg");
	if (usb_psy == NULL) {
		pd_err("%s Couldn't get usb_psy\n", __func__);
		ret = -1;
	} else {
