...
static int _pe2_is_algo_ready(struct chg_alg_device *alg)
{
	...
	if (pe2->charging_current_limit1 > 3000000) {
	...
	case PE2_HW_READY:
		...
		if (pe2_hal_get_charger_type(alg) !=
			...
			pr_err("%s %d %d -1", __func__, pe2->charging_current_limit1, pe2->charging_current_limit2);
	...
	do {
		...
		case PE2_HW_READY:
			...
			if (ret == 0) {
				...
				pe2->charging_current_limit2 != -1) {
				ret_value = ALG_NOT_READY;
				pr_err("%s %d %d -2", __func__, pe2->charging_current_limit1, pe2->charging_current_limit2);
			}else {
