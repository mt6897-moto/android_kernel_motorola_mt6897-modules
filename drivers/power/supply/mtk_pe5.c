...
#define PE50_IBUSOCP_RATIO	120
//-PERIDOT-35, liyiying.wt, 20240305, mod, debug 45w requirement
#define PE50_VBATOVP_RATIO	110
#define PE50_IBATOCP_RATIO	115
#define PE50_ITAOCP_RATIO	110
#define PE50_IBUSUCPF_RECHECK		250	/* mA */
#define PE50_VBUS_CALI_THRESHOLD	250	/* mV */
...
#define PE50_CHG_TERM_VTA	9000	/* mV */
#define PE50_CHG_TERM_INIT	3000	/* mA */

#define PE50_BATT_FFC_FV_10mv	10
#define PE50_BATT_FFC_FV_15mv	15
#define PE50_BATT_FFC_FV_20mv	20
#define PE50_BATT_FFC_FV_30mv	30
#define PE50_BATT_FFC_FV_40mv	40
#define PE50_BATT_FFC_FV_50mv	50
#define PE50_BATT_FFC_FV_60mv	60

#define DVCHG_TERM_COUNT_MAX	6
#define SWCHG_AICR_DISABLE_CURRENT_MIN	2000
#define SWCHG_DISABLE_CURRENT_MIN	7000
#define SWCHG_ICHG_MAX	1500
#define SWCHG_ICHG_STEP	150
#define SWCHG_AICR_MAX	1500
#define SWCHG_AICR_STEP	150

#define DVCHG_CV_CURRENT_MIN		3000
#define DVCHG_CC_CURRENT_MAX	3500
#define SWCHG_DVCHG_DOUBLE_ENABLE

#define PE50_SWCHG_TERM_COUNT_MAX	6
#define PE50_TERM_SWCHG_POLLING_INTERVAL	5000	/* ms */
#define P98300_CHARGE_NORMAL_ITA_MAX		4500
#define P98300_CHARGE_CP_ITA_MAX	3900
#define P98300_CHARGE_BAD_ITA_MAX	3700
#define P98300_CHARGE_BAD1_ITA_MAX	3100
#define P98300_CHARGE_CURRENT_MAX	8900

#define P98300_MAINTENANCE_V10_LEVEL0_CV	4500
#define P98300_MAINTENANCE_V10_LEVEL1_CV	4480
#define P98300_MAINTENANCE_V10_LEVEL2_CV	4430
#define P98300_MAINTENANCE_V10_LEVEL3_CV	4340

#define P98300_LOW_TEMP_TERM	1200
...
static const char *const pe50_algo_state_name[PE50_ALGO_STATE_MAX] = {
	"INIT", "MEASURE_R", "SS_SWCHG", "SS_DVCHG", "CC_CV", "TERM_SWCHG", "STOP",
};
...
static inline void pe50_calculate_vbat_ircmp(struct pe50_algo_info *info)
{
	...
	PE50_DBG("vbat_ircmp(vclamp,ibat,rbat)=%d(%d,%d,%d)\n",
...
out:
	PE50_DBG("vbat_cv(org,limit,no_ircmp,low_bound)=%d(%d,%d,%d,%d)\n",
...
static int pe50_set_dvchg_protection(struct pe50_algo_info *info)
{
	...
	ret = pe50_hal_set_vbatovp_alarm(info->alg, DVCHG1,
		data->is_swchg_en ? desc->swchg_off_vbat : (data->vbat_cv + 100));
	if (ret < 0) {
...
static int pe50_stop(struct pe50_algo_info *info, struct pe50_stop_info *sinfo)
{
	...
	*/
	...
	//+PERIDOT+35, yangpingao.wt, 20240320, mod, disable swchg when charge full
	if (do_reset && (data->chg_is_done != true)) {
	...
	if (data->chg_is_done != true) {
		pe50_enable_swchg_charging(info, true);
		pe50_hal_enable_sw_vbusovp(info->alg, true);
	}
	//-PERIDOT+35, yangpingao.wt, 20240320, mod, disable swchg when charge full

	//+PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp
	ret = pe50_hal_enable_batt_ircomp(info->alg, CHG1, true);
	if (ret < 0) {
		PE50_ERR("en ircomp fail(%d)\n", ret);
		return ret;
	}
	//-PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp

	//+PERIDOT+6476, yangpingao.wt, 20240524, mod, disable termination
	ret = pe50_hal_enable_termination(info->alg, CHG1, true);
	if (ret < 0) {
		PE50_ERR("en termination fail(%d)\n", ret);
		return ret;
	}
	//-PERIDOT+6476, yangpingao.wt, 20240524, mod, disable termination
...
static int pe50_calculate_rcable_by_swchg(struct pe50_algo_info *info)
{
	...
	for (i = 0; i < PE50_MEASURE_R_AVG_TIMES + 2; i++) {
		...
		vbus1 += val_vbus;
		ibus1 += val_ibus;
		PE50_DBG("vbus=%d ibus=%d vbus(max,min)=(%d,%d) ibus(max,min)=(%d,%d) vbus1=%d ibus1=%d",
	...
	for (i = 0; i < PE50_MEASURE_R_AVG_TIMES + 2; i++) {
		...
		vbus2 += val_vbus;
		ibus2 += val_ibus;
		PE50_DBG("vbus=%d ibus=%d vbus(max,min)=(%d,%d) ibus(max,min)=(%d,%d) vbus2=%d ibus2=%d",
...
static int pe50_algo_init_with_ta_cv(struct pe50_algo_info *info)
{
	...
	//+PERIDOT-35, yangpingao.wt, 20240413, mod, enable swchg power path before enable dvchg
	ret = pe50_hal_enable_hz(info->alg, CHG1, false);
	if (ret < 0) {
		PE50_ERR("disable swchg hz fail(%d)\n", ret);
		return ret;
	...
	ret = pe50_hal_set_aicr(info->alg, CHG1, 1000);
	if (ret < 0) {
		PE50_ERR("set aicr fail(%d)\n", ret);
		return ret;
	}
	//-PERIDOT-35, yangpingao.wt, 20240305, mod, enable swchg power path before enable dvchg
...
static int pe50_algo_init(struct pe50_algo_info *info)
{
	...
	//+PERIDOT-35, yangpingao.wt, 20240305, mod, disable dvchg when ibat <= idvchg_term
	data->dvchg_term_count = 0;
	data->swchg_term_count = 0;
	//-PERIDOT-35, yangpingao.wt, 20240305, mod, disable dvchg when ibat <= idvchg_term

	//+PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp
	ret = pe50_hal_enable_batt_ircomp(info->alg, CHG1, false);
	if (ret < 0) {
		PE50_ERR("disable ircomp fail(%d)\n", ret);
		return ret;
	}
	//-PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp
...
static int pe50_select_ita_lmt_by_r(struct pe50_algo_info *info, bool dual)
{
	...
	PE50_ERR("r_cable: %d\n", data->r_cable);
out:
	//+PERIDOT-35, yangpingao.wt, 20240320, mod, limit ita_max when charger at high temperature
	if (auth_data->ita_max == P98300_CHARGE_NORMAL_ITA_MAX ||
		auth_data->ita_max == P98300_CHARGE_BAD_ITA_MAX) {
		data->ita_lmt = auth_data->ita_max -100;
	}
	data->ita_lmt = min(ita_lmt_by_r, data->ita_lmt);
	PE50_INFO("ita limited by r = %d, ita_lmt = %d\n", ita_lmt_by_r, data->ita_lmt);
	//-PERIDOT-35, yangpingao.wt, 20240320, mod, limit ita_max when charger at high temperature
...
single_dvchg_select_ita:
	...
	//+PERIDOT-11917, yangpingao.wt, 20240702, mod, set swchg min ichg
	...
	//+PERIDOT-35, yangpingao.wt, 20240305, mod, dvchg cc current limit
	ret = pe50_hal_get_vbat(info->alg, &vbat);
	if (ret < 0) {
		PE50_ERR("get vbat fail(%d)\n", ret);
		goto out;
	}
	...
	PE50_INFO("batt(ibat:%d,vbat:%d), cp(vbus:%d, ibus:%d), (vta_step, ita_step) = (%d,%d)\n",
		 ibat, vbat, data->vta_measure, data->ita_measure,
		 auth_data->vta_step, ita_gap_per_vstep);
	//-PERIDOT-35, yangpingao.wt, 20240305, mod, dvchg cc current limit

	/* VBAT reaches CV level */
	PE50_DBG("vbat(%d), ita(%d)\n", vbat, data->ita_measure);
	if (vbat >= data->vbat_cv) {
		if (data->ita_measure < data->idvchg_term &&
		    data->is_dvchg_en[PE50_DVCHG_SLAVE]) {
			ret = pe50_check_slave_dvchg_off(info);
		...
		//+PERIDOT-35, yangpingao.wt, 20240305, mod, set swchg min ichg
		data->swchg_set_ichg = SWCHG_ICHG_STEP;
		ret = pe50_hal_set_ichg(info->alg, CHG1, data->swchg_set_ichg);
		if (ret < 0) {
			PE50_ERR("set ichg fail(%d)\n", ret);
			goto out;
		}

		data->swchg_set_aicr = SWCHG_AICR_STEP;
		ret = pe50_hal_set_aicr(info->alg, CHG1, data->swchg_set_aicr);
		if (ret < 0) {
			PE50_ERR("set aicr fail(%d)\n", ret);
			goto out;
		}
	...
	idvchg_lmt = pe50_get_idvchg_lmt(info);
	//+PERIDOT-35, yangpingao.wt, 20240305, mod, dvchg cc current limit
	idvchg_lmt = min(idvchg_lmt, (u32)DVCHG_CC_CURRENT_MAX);
	//-PERIDOT-35, yangpingao.wt, 20240305, mod, dvchg cc current limit

	/* ITA reaches CC level */
	if ((data->ita_measure + 250) > idvchg_lmt ||
		ibat >= (idvchg_lmt * 2 - 500) ||
		ibat >= abs(data->ibat_current_limit - 1000) ||
		vta == auth_data->vcap_max) {
		data->state = PE50_ALGO_CC_CV;
...
static int pe50_algo_term_swchg(struct pe50_algo_info *info)
{
	int ret, vbat = 0, ibat = 0;
	u32 soc = 0;
	int iswchg_term = 0;
	struct pe50_algo_desc *desc = info->desc;
	struct pe50_algo_data *data = info->data;
	...
	if (data->term_limit >= 800) {
		iswchg_term = 800;
	} else {
		iswchg_term = 500;
	}
	ret = pe50_hal_set_eoc_current(info->alg, CHG1, iswchg_term);
	if (ret < 0) {
		PE50_ERR("set ieoc fail(%d)\n", ret);
	}

	ret = pe50_hal_charge_full_check(info->alg, CHG1, (data->term_limit - 50));
	if (ret < 0) {
		PE50_ERR("swchg full check fail(%d)\n", ret);
	}

	if (vbat > (data->vbat_cv - desc->vbat_max_gap) &&
		ibat < (data->term_limit + 50)) {
		if (data->swchg_term_count++ >= PE50_SWCHG_TERM_COUNT_MAX) {
			ret = pe50_hal_enable_charging(info->alg, CHG1, false);
			if (ret < 0) {
				PE50_ERR("en swchg fail(%d)\n", ret);
				goto out;
			...
			ret = pe50_enable_dvchg_charging(info, PE50_DVCHG_MASTER, false);
			if (ret < 0) {
				PE50_ERR("disable dvchg fail(%d)\n", ret);
				goto out;
			}

			mdelay(50);
			ret = pe50_hal_set_constant_voltage(info->alg, CHG1, data->init_cv_limit);
			if (ret < 0) {
				PE50_ERR("set swchg cv fail(%d)\n", ret);
				goto out;
			}

			data->chg_is_done = true;
			ret = pe50_set_ta_cap_cv(info, PE50_VTA_INIT, PE50_ITA_INIT);
			if (ret < 0) {
			...
			PE50_INFO("pe5_charge_full !!!\n");
		}
	}

	PE50_INFO("swchg_exit_check:(vbat, ibat, idvchg_term, term_count) = (%d, %d, %d, %d)\n",
					vbat, ibat, data->term_limit, data->swchg_term_count);
...
{
	int ret,  vbat, vbat_cp, vsys = 0;
	int ibat, swchg_ibat, sw_ibus_meas = 0;
	struct pe50_algo_desc *desc = info->desc;
	struct pe50_algo_data *data = info->data;
	struct pe50_ta_auth_data *auth_data = &data->ta_auth_data;
	...
	ret = pe50_get_adc(info, PE50_ADCCHAN_VBAT, &vbat_cp);
	if (ret < 0) {
		PE50_ERR("get vbat fail(%d)\n", ret);
		goto out;
	}
...
//+PERIDOT-35, yangpingao.wt, 20240305, mod, swchg and dvchg double enable
#ifdef SWCHG_DVCHG_DOUBLE_ENABLE
	ret = pe50_hal_get_adc(info->alg, CHG1, PE50_ADCCHAN_IBUS, &sw_ibus_meas);
	if (ret < 0) {
		PE50_ERR("get swchg ibus fail(%d)\n", ret);
		goto out;
	}
	...
	swchg_ibat = ibat -(data->ita_measure * 2);
	PE50_INFO("batt(ibat:%d,vbat_cp:%d, vbat_gauge:%d), swchg(ibat:%d, ibus_meas:%d), cp(vbus:%d, ibus:%d)\n",
		 ibat, vbat_cp, vbat, swchg_ibat, sw_ibus_meas,
		 data->vta_measure, data->ita_measure);

	PE50_DBG("(vta_step,ita_step) = (%d,%d)\n", auth_data->vta_step, ita_gap_per_vstep);

	//swchg enable power path
	if (data->ibat_current_limit <= SWCHG_AICR_DISABLE_CURRENT_MIN)  {
		if (data->swchg_set_aicr > SWCHG_AICR_STEP) {
			data->swchg_set_aicr -= SWCHG_AICR_STEP;
		}
	} else {
		if (data->swchg_set_aicr < SWCHG_AICR_MAX) {
			data->swchg_set_aicr += SWCHG_ICHG_STEP;
		}
	}

	PE50_DBG("power_path: set_aicr = %d\n", data->swchg_set_aicr);
	ret = pe50_hal_set_aicr(info->alg, CHG1, data->swchg_set_aicr);
	if (ret < 0) {
		PE50_ERR("set aicr fail(%d)\n", ret);
		return ret;
	...
	ret = pe50_hal_set_constant_voltage(info->alg, CHG1, data->jeita_cv_limit + 130);
	if (ret < 0) {
		PE50_ERR("set swchg cv fail(%d)\n", ret);
		goto out;
	}

	//swchg enable or disable
	if ((data->ibat_current_limit <= SWCHG_DISABLE_CURRENT_MIN) ||
		(vbat >= (data->vbat_cv -20) && ibat <= SWCHG_DISABLE_CURRENT_MIN) ||
		(idvchg_lmt <= P98300_CHARGE_BAD1_ITA_MAX)) {
		if (data->swchg_set_ichg >= SWCHG_ICHG_STEP) {
			data->swchg_set_ichg -= SWCHG_ICHG_STEP;
		}
		PE50_DBG("swchg_disable: set_ichg = %d\n", data->swchg_set_ichg);
		...
		if (data->swchg_set_ichg < SWCHG_ICHG_MAX) {
			data->swchg_set_ichg += SWCHG_ICHG_STEP;
		}
		PE50_DBG("swchg_enable: set_ichg = %d\n", data->swchg_set_ichg);
	...
	// disable dvchg when ibat <= idvchg_term
	if (vbat > (data->vbat_cv - desc->vbat_max_gap) &&
		ibat < (data->idvchg_term * 2)) {
		if (data->dvchg_term_count++ >= DVCHG_TERM_COUNT_MAX) {
			//swchg enable power path
			...
			ret = pe50_hal_set_ichg(info->alg, CHG1, (data->idvchg_term * 2 + 50));
			if (ret < 0) {
				PE50_ERR("set ichg fail(%d)\n", ret);
				goto out;
			}
			...
			//swchg enable
			//mdelay(100);
			ret = pe50_hal_set_constant_voltage(info->alg, CHG1, data->jeita_cv_limit);
			if (ret < 0) {
				PE50_ERR("set swchg cv fail(%d)\n", ret);
				goto out;
			}
			...
			ret = pe50_set_ta_cap_cv(info, PE50_CHG_TERM_VTA, PE50_CHG_TERM_INIT);
			if (ret < 0) {
				PE50_ERR("set_ta_cap fail(%d)\n", ret);
				sinfo.hardreset_ta = true;
				goto out;
			...
			data->state = PE50_ALGO_TERM_SWCHG;
			return 0;
		}

		PE50_INFO("dvchg_exit_check:(vbat, ibat, idvchg_term, term_count) = (%d, %d, %d, %d)\n",
						vbat, ibat, data->idvchg_term, data->dvchg_term_count);
	}

	if (vbat >= data->vbat_cv) {
		PE50_DBG("--vbat >= vbat_cv, %d > %d\n", vbat, data->vbat_cv);
		vta -= auth_data->vta_step;
		ita -= ita_gap_per_vstep;
		data->is_vbat_over_cv = true;
	} else if (ibat > (data->ibat_current_limit + 50) || vsys >= PE50_VSYS_UPPER_BOUND ||
		   idvchg_lmt < ita_gap_per_vstep || data->ita_measure > P98300_CHARGE_CP_ITA_MAX) {
		vta -= auth_data->vta_step;
		ita -= ita_gap_per_vstep;
		PE50_DBG("--vta, ita(meas,lmt)=(%d,%d)\n", data->ita_measure,
			  idvchg_lmt);
	} else if (!data->is_vbat_over_cv && vbat <= data->cv_lower_bound &&
		   ibat <= (data->ibat_current_limit - 100) &&
...
//+PERIDOT-3013, yangpingao.wt, 20240418, mod, dvchg cc current limit
	ita = max(ita, (u32)DVCHG_CV_CURRENT_MIN);
//-PERIDOT-3013, yangpingao.wt, 20240418, mod, dvchg cc current limit
...
#ifdef SWCHG_DVCHG_DOUBLE_ENABLE
	if (data->chg_is_done == true) {
		PE50_INFO("pe5 charging done !!!: soc = %d\n", soc);
		return false;
	}
#else
...
static void pe50_algo_data_partial_reset(struct pe50_algo_info *info)
{
	...
	data->cv_limit = -1;
	//+PERIDOT-35, yangpingao.wt, 20240305, mod, disable dvchg when ibat <= idvchg_term
	data->jeita_cv_limit = 4250;
	data->init_cv_limit = 4250;
	data->term_limit = 1700;
	data->chg_is_done = false;
	//-PERIDOT-35, yangpingao.wt, 20240305, mod, disable dvchg when ibat <= idvchg_term
	mutex_unlock(&data->ext_lock);
	data->start_soc_max = desc->start_soc_max;
}
...
static int pe50_dump_charging_info(struct pe50_algo_info *info)
{
	...
	PE50_DBG("vbus,ibus(master,slave,sw),vbat,ibat=%d,(%d,%d,%d),%d,%d\n",
		 vbus, ibus[PE50_DVCHG_MASTER], ibus[PE50_DVCHG_SLAVE],
		 ibus_swchg, vbat, ibat);
	PE50_DBG("vta,ita(set,meas)=(%d,%d),(%d,%d),force_cv=%d\n",
		 data->vta_setting, data->vta_measure, data->ita_setting,
		 data->ita_measure, data->force_ta_cv);
	PE50_DBG("vout(master,slave)=(%d,%d)\n",
		 vout[PE50_DVCHG_MASTER], vout[PE50_DVCHG_SLAVE]);
	PE50_DBG("[PE5] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
...
static int pe50_algo_threadfn(void *param)
{
	...
	while (!kthread_should_stop()) {
		...
		case PE50_ALGO_CC_CV:
			...
			break;
		case PE50_ALGO_TERM_SWCHG:
			pe50_algo_term_swchg(info);
			break;
		case PE50_ALGO_STOP:
		...
		if (data->state != PE50_ALGO_STOP) {
			...
			} else if (data->state == PE50_ALGO_TERM_SWCHG) {
				polling_interval = PE50_TERM_SWCHG_POLLING_INTERVAL;
			} else {
				  ...
				  struct chg_limit_setting *setting)
{
	...
	//+PERIDOT-35, yangpingao.wt, 20240305, mod, charging current dynamic adjust by thermal
	int ibat = micro_to_milli(setting->charging_current_limit1);
	int term = micro_to_milli(setting->term);

	mutex_lock(&data->ext_lock);
	if (data->cv_limit != cv || data->input_current_limit != ic
		|| data->ibat_current_limit != ibat || data->term_limit != term) {
		data->cv_limit = cv;
		data->jeita_cv_limit = cv;
		data->init_cv_limit = cv;
		if (data->cv_limit >= P98300_MAINTENANCE_V10_LEVEL0_CV) {
			data->cv_limit += PE50_BATT_FFC_FV_15mv;
			if (data->term_limit <= P98300_LOW_TEMP_TERM) {
				data->jeita_cv_limit += PE50_BATT_FFC_FV_40mv;
			} else {
				data->jeita_cv_limit += PE50_BATT_FFC_FV_50mv;
			}
		} else if (data->cv_limit < P98300_MAINTENANCE_V10_LEVEL0_CV &&
			data->cv_limit >= P98300_MAINTENANCE_V10_LEVEL1_CV) {
			if (data->term_limit <= P98300_LOW_TEMP_TERM) {
				data->jeita_cv_limit += PE50_BATT_FFC_FV_40mv;
			} else {
				data->jeita_cv_limit += PE50_BATT_FFC_FV_50mv;
			}
		} else if (data->cv_limit < P98300_MAINTENANCE_V10_LEVEL1_CV &&
			data->cv_limit >= P98300_MAINTENANCE_V10_LEVEL2_CV) {
			if (data->term_limit <= P98300_LOW_TEMP_TERM) {
				data->jeita_cv_limit += PE50_BATT_FFC_FV_40mv;
			} else {
				data->jeita_cv_limit += PE50_BATT_FFC_FV_50mv;
			}
		} else if (data->cv_limit < P98300_MAINTENANCE_V10_LEVEL2_CV &&
			data->cv_limit >= P98300_MAINTENANCE_V10_LEVEL3_CV) {
			if (data->term_limit <= P98300_LOW_TEMP_TERM) {
				data->jeita_cv_limit += PE50_BATT_FFC_FV_40mv;
			} else {
				data->jeita_cv_limit += PE50_BATT_FFC_FV_50mv;
			...
			data->cv_limit -= PE50_BATT_FFC_FV_40mv;
			data->jeita_cv_limit += PE50_BATT_FFC_FV_20mv;
		}

		data->input_current_limit = ic;
		data->ibat_current_limit = ibat;
		data->term_limit = term;
		if (data->ibat_current_limit >= P98300_CHARGE_CURRENT_MAX) {
			data->ibat_current_limit = P98300_CHARGE_CURRENT_MAX;
		}
		PE50_INFO("input_current = %d, ibat = %d, vbat_cv = %d, init_cv = %d, term = %d\n",
			ic, ibat, data->cv_limit, data->init_cv_limit, term);
		pe50_wakeup_algo_thread(data);
	}
	mutex_unlock(&data->ext_lock);
	//-PERIDOT-35, yangpingao.wt, 20240305, mod, charging current dynamic adjust by thermal
			 ...
			 enum chg_alg_props s, int *value)
{
	...
	case ALG_MAX_VBUS:
		*value = auth_data->vcap_max;
		break;
	//+PERIDOT-35, yangpingao.wt, 20240328, mod, get pe5 charge done
	case ALG_CHG_DONE:
		*value = (int)data->chg_is_done;
		break;
	//-PERIDOT-35, yangpingao.wt, 20240328, mod, get pe5 charge done
	default:
		break;
