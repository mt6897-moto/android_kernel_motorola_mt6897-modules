...
enum pe50_algo_state {
	...
	PE50_ALGO_SS_DVCHG,
	PE50_ALGO_CC_CV,
	PE50_ALGO_TERM_SWCHG,	//PERIDOT-35, yangpingao.wt, 20240305, mod,  check swchg charge full
	PE50_ALGO_STOP,
	PE50_ALGO_STATE_MAX,
};
...
struct pe50_algo_data {
	...
	int ibat_current_limit;
	int cv_limit;
	u32 start_soc_max;		/* algo start soc upper bound */
	bool chg_is_done;	//PERIDOT-35, yangpingao.wt, 20240305, mod,  check swchg charge full
	int term_limit;
	int dvchg_term_count;
	int swchg_term_count;
	int swchg_set_ichg;
	int swchg_set_aicr;
	int jeita_cv_limit;
	int init_cv_limit;
...
extern int pe50_hal_charge_full_check(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mA);
extern int pe50_hal_set_eoc_current(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mA);
extern int pe50_hal_set_constant_voltage(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mV);
extern int pe50_hal_get_vbat(struct chg_alg_device *alg, u32 *vbat);
//+PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp
extern int pe50_hal_enable_batt_ircomp(struct chg_alg_device *alg, enum chg_idx chgidx,
			     bool en);
