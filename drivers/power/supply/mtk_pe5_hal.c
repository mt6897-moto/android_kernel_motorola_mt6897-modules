			     ...
			     struct pe50_ta_auth_data *data)
{
	...
	mtk_master_charger_psy = power_supply_get_by_name("mtk-master-charger");
	...
	for (i = 0; i < hal->support_ta_cnt; i++) {
		...
		//+Peridot-1559, liyiying.wt, 20240522, mod, when charging with a 30W charger charging process is intermitten
		if (data->pwr_lmt == 0 && data->pdp == 0) {
			if (mtk_master_charger_psy != NULL) {
				info = (struct mtk_charger *)power_supply_get_drvdata(mtk_master_charger_psy);
...
int pe50_hal_enable_batt_ircomp(struct chg_alg_device *alg, enum chg_idx chgidx,
			     bool en)
{
	...
	if (chgtyp < 0)
		return chgtyp;
	return charger_dev_set_bat_ircomp(hal->chgdevs[chgtyp], en);
}
//-PERIDOT-7084, yangpingao.wt, 20240502, mod, disable batt ircomp

//+PERIDOT+6476, yangpingao.wt, 20240524, mod, disable termination
int pe50_hal_enable_termination(struct chg_alg_device *alg, enum chg_idx chgidx,
			     bool en)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pe50_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
int pe50_hal_get_vbat(struct chg_alg_device *alg, u32 *vbat)
{
	int ret = -EOPNOTSUPP;
	union power_supply_propval val = {0,};
	struct pe50_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
...
//+PERIDOT+35, yangpingao.wt, 20240305, mod, disable dvchg when ibat <= idvchg_term
int pe50_hal_charge_full_check(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mA)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pe50_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
	...
	return charger_dev_safety_check(hal->chgdevs[chgtyp], milli_to_micro(mA));
}

int pe50_hal_set_eoc_current(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mA)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pe50_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
	...
	if (chgtyp < 0)
		return chgtyp;
	return charger_dev_set_eoc_current(hal->chgdevs[chgtyp],
						milli_to_micro(mA));
}

int pe50_hal_set_constant_voltage(struct chg_alg_device *alg, enum chg_idx chgidx, u32 mV)
{
	int chgtyp = to_chgtyp(chgidx);
	struct pe50_hal *hal = chg_alg_dev_get_drv_hal_data(alg);
