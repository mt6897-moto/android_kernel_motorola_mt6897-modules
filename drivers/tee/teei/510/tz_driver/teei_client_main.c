...
//chenluowu modify for improve fingerprint unlock speed
...
struct shm_handle_param {
	unsigned long size;
	unsigned long handle_id;
};

struct shm_handle_list {
	unsigned long size;
	unsigned long handle_id;
	unsigned long addr;
...
int teei_set_switch_pri(unsigned long policy)
{
	...
	//IMSG_ERROR("0529 super big teei_set_switch_pri start policy is %lx \n", policy);
	if (policy == TEEI_RT_POLICY) {
		if (teei_switch_task != NULL) {
			//IMSG_ERROR("teei_set_switch_pri   TEEI_RT_POLICY  \n");
			set_user_nice(teei_switch_task, MIN_NICE + 1);
			return 0;
		} else
			return -EINVAL;
	} else if (policy == TEEI_NORMAL_POLICY) {
		if (teei_switch_task != NULL) {
			//IMSG_ERROR("teei_set_switch_pri   TEEI_NORMAL_POLICY  \n");
	...
	//IMSG_ERROR("teei_set_switch_pri end\n");
	return retVal;
#else
	return 0;
#endif
...
int get_current_teei_cpuid(void)
{
...
{
	unsigned int curr = get_current_teei_cpuid();
...
int handle_switch_core(int cpu)
{
	...
	IMSG_PRINTK("change cpu id from %d(0x%lx) to %d(0x%lx)\n",
			cpu, (unsigned long)teei_cpu_id[cpu],
			switch_to_cpu_id, (unsigned long)teei_cpu_id[switch_to_cpu_id]);
...
static int nq_cpu_up_prep(unsigned int cpu)
{
	...
	int retVal = 0;
	unsigned int sched_cpu = get_current_teei_cpuid();
...
static void boot_stage1(unsigned long vfs_addr, unsigned long tlog_addr)
{
	...
	if (retVal != 0) {
		IMSG_ERROR("[%s][%d] TEEI: Failed to register tlog %d!\n",
	...
	retVal = add_work_entry(SMC_CALL_TYPE, N_INIT_T_BOOT_STAGE1,
			soter_sec_world_id[SOTER_SEC_WORLD_ID_TLOG],
			soter_sec_world_id[SOTER_SEC_WORLD_ID_TLOG] >> 32);
	if (retVal != 0) {
		IMSG_ERROR("[%s][%d] TEEI: Failed to add_work_entry %d!\n",
				__func__, __LINE__, retVal);
		return;
	}
...
static long teei_alloc_shm_handle(unsigned long arg)
{
	...
	struct shm_handle_param param;
	unsigned long size = 0;
	unsigned long handle_id = 0;
	void *shm_buff = NULL;
	long retVal = 0;

	retVal = copy_from_user(&param, (void *)arg,
			sizeof(struct shm_handle_param));
	if (retVal != 0) {
		IMSG_ERROR("Failed to copy shm_handle_param from user\n");
		retVal = -EINVAL;
		goto out;
	}
	...
	entry = kmalloc(sizeof(struct shm_handle_list), GFP_KERNEL);
	if (entry == NULL) {
		IMSG_ERROR("Failed to alloc shm_handle_list!\n");
		retVal = -ENOMEM;
		goto free_shm_buff;
	}
	...
	entry->size = ROUND_UP(size, SZ_4K);
	entry->addr = (unsigned long)shm_buff;

	mutex_lock(&shm_handle_mutex);
	list_add_tail(&(entry->list), &g_shm_handle_link);
	mutex_unlock(&shm_handle_mutex);

	param.handle_id = handle_id;

	retVal = copy_to_user((void *)arg, &param,
				sizeof(struct shm_handle_param));

	if (retVal != 0) {
		IMSG_ERROR("Failed to copy handle ID to user, %ld\n", retVal);
		goto del_list;
	}
...
static long teei_release_shm_handle(unsigned long arg)
{
	struct shm_handle_list *entry = NULL;
	struct shm_handle_param param;
	unsigned long size = 0;
	...
	list_for_each_entry(entry, &g_shm_handle_link, list) {
		if (entry->handle_id == handle_id) {
			shm_buff = (void *)(entry->addr);
			size = entry->size;
			list_del(&(entry->list));
			break;
		}
	...
	if (shm_buff == NULL) {
		IMSG_ERROR("Failed to find handle ID in shm_handle_list.\n");
		retVal = -EINVAL;
		goto out;
	}
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case TEEI_CONFIG_IOCTL_ALLOC_HANDLE:
		...
		if (retVal != 0) {
			IMSG_ERROR("Failed to teei_alloc_shm_handle %d\n",
					retVal);
		}
		break;

	case TEEI_CONFIG_IOCTL_RELEASE_HANDLE:
		retVal = teei_release_shm_handle(arg);
		if (retVal != 0) {
			IMSG_ERROR("Failed to teei_release_shm_handle %d\n",
					retVal);
		}
		break;
...
#endif
	cpumask_set_cpu(get_current_teei_cpuid(), &mask);
	set_cpus_allowed_ptr(teei_switch_task, &mask);
