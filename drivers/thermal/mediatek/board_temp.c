...
struct board_ntc_info {
	...
	void __iomem *en_reg;
	struct pmic_auxadc_data *adc_data;
	struct iio_channel *chan_usb_board_ntc;
	struct iio_channel *chan_flash_led_ntc;
	struct iio_channel *chan_main_board_ntc;
};
...
static int board_ntc_get_temp(struct thermal_zone_device *tz, int *temp)
{
	...
	//+PERIDOT-426, mashaokang.wt, modify, 20240206, read the ADC value from io-channel if channel-name is matched
	if (!PTR_ERR_OR_ZERO(ntc_info->chan_usb_board_ntc)){
		iio_read_channel_raw(ntc_info->chan_usb_board_ntc, &val);
		r_type = 0;
	} else if(!PTR_ERR_OR_ZERO(ntc_info->chan_flash_led_ntc)){
		iio_read_channel_raw(ntc_info->chan_flash_led_ntc, &val);
		r_type = 0;
	} else if(!PTR_ERR_OR_ZERO(ntc_info->chan_main_board_ntc)){
		iio_read_channel_raw(ntc_info->chan_main_board_ntc, &val);
		r_type = 0;
	//-PERIDOT-426, mashaokang.wt, modify, 20240206, read the ADC value from io-channel if channel-name is matched
	} else {
...
static int board_ntc_probe(struct platform_device *pdev)
{
	...
	//+PERIDOT-426, mashaokang.wt, modify, 20240219, read the ADC value from io-channel if channel-name is matched
	ntc_info->chan_usb_board_ntc = devm_iio_channel_get(&pdev->dev, "USB_BOARD_NTC");
	ntc_info->chan_flash_led_ntc = devm_iio_channel_get(&pdev->dev, "FLASH_LED_NTC");
	ntc_info->chan_main_board_ntc = devm_iio_channel_get(&pdev->dev, "MAIN_BOARD_NTC");
	//-PERIDOT-426, mashaokang.wt, modify, 20240219, read the ADC value from io-channel if channel-name is matched
