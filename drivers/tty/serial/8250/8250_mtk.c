...
#endif
/*BEGIN TINNO, 20230215，guoxiong2, add for keyboard  feature*/
#include <linux/tn_common.h>
tn_keyboard_uart_dma_call_back_t uart_dma_call_back = NULL;
EXPORT_SYMBOL(uart_dma_call_back);
tn_keyboard_uart_dma_set_call_back_t uart_dma_set_call_back = NULL;
EXPORT_SYMBOL(uart_dma_set_call_back);
tn_keyboard_uart_dma_write_call_back_t uart_dma_write_call_back = NULL;
EXPORT_SYMBOL(uart_dma_write_call_back);
...
//+peridot-4533,fangzhihua.wt,add, 20240419,fix uart3 changed cause pogo keyboard fail
static unsigned int uart3_def_enable_addr;
static void __iomem *uart3_def_addr;
//-peridot-4533,fangzhihua.wt,add, 20240419,fix uart3 changed cause pogo keyboard fail
...
/*BEGIN TINNO, 2024427，longqing, add for keyboard  feature*/
static void pogo_dma_tx_complete(void *param);
...
int pogo_tx_dma(struct uart_8250_port *p){
	...
	dma->tx_running = 1;
	desc->callback = pogo_dma_tx_complete;
...
static void pogo_dma_tx_complete(void *param){
	...
	if(uart_dma_write_call_back != NULL){
		uart_dma_write_call_back(0);           
	}else{
		pr_err("TN_KB:%s %d callback failed!\n",__func__,__LINE__);
	}
	spin_unlock_irqrestore(&p->port.lock, flags);
}
/*END TINNO, 2024427，longqing, add for keyboard  feature*/
...
static void mtk8250_dma_rx_complete(void *param)
{
	...
	int copied, total, cnt, copied_sec = 0;
	unsigned char *ptr;
	unsigned long flags;
...
static int mtk8250_startup(struct uart_port *port)
{
    ...
    if(uart_dma_set_call_back != NULL){
        uart_dma_set_call_back(up, 1);
    }else{
        pr_err("TN_KB:%s %d callback failed!\n",__func__,__LINE__);
    }
    uart3_def_addr = ioremap(uart3_def_enable_addr, 0x10);
    pr_notice("[%s] first get uart3_def_value = 0x%0x\n", __func__, UART_REG_READ(uart3_def_addr));
    UART_REG_WRITE(uart3_def_addr, UART_REG_READ(uart3_def_addr) | 0x10);
    //+peridot-5855, fangzhihua.wt, 20240516,fix uart current big
    UART_REG_WRITE(uart3_def_addr, UART_REG_READ(uart3_def_addr) & 0xFE);
    //-peridot-5855, fangzhihua.wt, 20240516,fix uart current big
    pr_notice("[%s] second get uart3_def_value = 0x%0x\n", __func__, UART_REG_READ(uart3_def_addr));
    /*END TINNO, 2024427，longqing, add for keyboard  feature*/
			   ...
			   struct mtk8250_data *data)
{
	...
	//+keyboard UART3,fangzhihua.wt,20240224, add mtk patch
	int uart3_def_enable;
	//+peridot-4533,fangzhihua.wt,add, 20240419,fix uart3 changed cause pogo keyboard fail
	//unsigned int uart3_def_enable_addr;
	//void __iomem *uart3_def_addr;
	//-peridot-4533,fangzhihua.wt,add, 20240419,fix uart3 changed cause pogo keyboard fail
	int ret = 0;
	//-keyboard UART3,fangzhihua.wt,20240224, add mtk patch
	...
	//+keyboard UART3,fangzhihua.wt,20240224, add mtk patch
	ret = of_property_read_u32_index(pdev->dev.of_node,"uart3-def-enable", 0, &uart3_def_enable);                    
	pr_notice("[%s] first get uart3_def_enable = %d, ret = %d\n", __func__, uart3_def_enable, ret);
 
	if (!ret && uart3_def_enable == 1) {
		ret = of_property_read_u32_index(pdev->dev.of_node,"uart3-def-addr", 0, &uart3_def_enable_addr);
		if (ret) {
			pr_notice("[%s] get uart3-def-addr addr fail\n", __func__);
			return -1;
		}
...
static int mtk8250_probe(struct platform_device *pdev)
{
	...
	if (data->dma) {
		...
		pr_err("TN_KB:%s %d set pogo_tx_dma ok!\n",__func__,__LINE__);
		uart.dma->tx_dma = pogo_tx_dma;
	}else{
	    pr_err("TN_KB:%s %d set pogo_tx_dma fail!\n",__func__,__LINE__);
	}
#endif
