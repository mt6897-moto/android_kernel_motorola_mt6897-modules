...
#define AW883XX_DRIVER_VERSION "v1.6.0_1"
...
#define AW883XX_ACF_FILE_0	"aw883xx_acf_gyt.bin"
#define AW883XX_ACF_FILE_1	"aw883xx_acf_yc.bin"
	...
	if (aw883xx->aw_pa->fw_status == AW_DEV_FW_OK) {
		...
		for (i = 0; i < AW_START_RETRIES; i++) {
			...
			if (ret < 0) {
				...
				if (sync_start == AW_SYNC_START || aw883xx->aw_pa->channel == 0)
					aw883xx_start_pa(aw883xx);
				else
...
static int aw883xx_request_firmware_file(struct aw883xx *aw883xx)
{
	...
	for (i = 0; i < AW_REQUEST_FW_RETRIES; i++) {
		if (aw883xx->spk_id_flag == 0) {
			ret = request_firmware(&cont, AW883XX_ACF_FILE_0, aw883xx->dev);
			aw_dev_info(aw883xx->dev, "id %d, load %s",
						aw883xx->spk_id_flag, AW883XX_ACF_FILE_0);
		} else {
			ret = request_firmware(&cont, AW883XX_ACF_FILE_1, aw883xx->dev);
			aw_dev_info(aw883xx->dev, "id %d, load %s",
						aw883xx->spk_id_flag, AW883XX_ACF_FILE_1);
		}
...
static int aw883xx_gpio_request(struct aw883xx *aw883xx)
{
	...
	if (gpio_is_valid(aw883xx->spk_id_gpio)) {
		ret = devm_gpio_request_one(aw883xx->dev, aw883xx->spk_id_gpio,
			GPIOF_DIR_IN, "aw883xx_spk_id");
		if (ret) {
			aw_dev_err(aw883xx->dev, "spk id request failed");
			return ret;
		}
		gpio_direction_input(aw883xx->spk_id_gpio);
		aw883xx->spk_id_flag = gpio_get_value(aw883xx->spk_id_gpio);
		gpio_free(aw883xx->spk_id_gpio);
	}
...
static int aw883xx_parse_gpio_dt(struct aw883xx *aw883xx)
{
	...
	aw883xx->spk_id_gpio = of_get_named_gpio(np, "spk-id-gpio", 0);
	if (aw883xx->spk_id_gpio < 0) {
		aw_dev_err(aw883xx->dev, "no spk gpio provided");
		return -EIO;
	} else {
		aw_dev_info(aw883xx->dev, "spk gpio provided ok");
	}
	return 0;
}
...
static ssize_t aw883xx_spk_id_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	struct aw883xx *aw883xx = dev_get_drvdata(dev);
	ssize_t len = 0;
	len += snprintf(buf + len, PAGE_SIZE - len,"spk id : %d\n", aw883xx->spk_id_flag);
	return len;
}
...
static DEVICE_ATTR(spk_id, S_IRUGO,aw883xx_spk_id_show, NULL);
