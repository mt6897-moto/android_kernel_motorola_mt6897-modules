...
extern __attribute__ ((weak)) ssize_t kernel_read(struct file *file, void *buf, size_t count, loff_t *pos);
...
#define AWINIC_CALI_FILE  "/mnt/vendor/persist/factory/aw_cali.bin"
#define AWINIC_CALI_F0_FILE  "/mnt/vendor/persist/factory/aw_cali_f0.bin"
...
static int aw_cali_write_cali_f0_to_file(int32_t cali_f0, int channel)
{
	struct file *fp = NULL;
	char buf[50] = { 0 };
	loff_t pos = 0;
	...
	fp = filp_open(AWINIC_CALI_F0_FILE, O_RDWR | O_CREAT, 0644);
	if (IS_ERR(fp)) {
		aw_pr_err("channel:%d open %s failed, error=%ld",
				channel, AWINIC_CALI_F0_FILE, PTR_ERR(fp));
		return -EINVAL;
	}
	pos = AW_INT_DEC_DIGIT * channel;

	snprintf(buf, sizeof(buf), "%10d", cali_f0);
	...
	aw_pr_info("channel:%d buf:%s cali_f0:%d",
			channel, buf, cali_f0);
...
static int aw_cali_write_f0_to_nvram(int32_t cali_f0, int32_t channel)
{
#ifdef AW_CALI_STORE_EXAMPLE
	return aw_cali_write_cali_f0_to_file(cali_f0, channel);
#else
	return 0;
#endif
}
...
static int aw_cali_svc_get_cali_f0_q(struct aw_device *aw_dev)
{
	...
	aw_cali_write_f0_to_nvram(cali_desc->f0, aw_dev->channel);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (is_single_cali) {
		...
		if ((aw_dev->cali_desc.cali_re >= 5250) && (aw_dev->cali_desc.cali_re <= 8750)){
			result = 1;
		}else{
			result = 0;
		}
		if (ret <= 0) {
			...
			len += snprintf(buf+len, PAGE_SIZE-len, ",result=%d\n",result);
		}
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (is_single_cali) {
		...
		if(((aw_dev->cali_desc.f0*10)>=4300)&&((aw_dev->cali_desc.f0*10)<=8300)){
			result=1;
		}else{
			result=0;
		}
