...
static int aw883xx_set_dsp_noise_gate_params(struct aw_device *aw_dev, uint32_t enable)
{
	int ret = -1;
	...
	aw_dev_dbg(aw_dev->dev, "%s dsp noise gate", enable ? "enable" : "disable");
	if (enable) {
		ret = aw_dev->ops.aw_reg_write_bits(aw_dev, dsp_ng_desc->reg,
				dsp_ng_desc->mask, dsp_ng_desc->enable);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "set enable failed");
			return ret;
		}
		...
		ret = aw_dev->ops.aw_reg_write_bits(aw_dev, dsp_ng_desc->reg,
				dsp_ng_desc->mask, dsp_ng_desc->disable);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "set disable failed");
			return ret;
		}
...
static int aw883xx_set_dsp_lowe_power_params(struct aw_device *aw_dev, uint32_t val)
{
	int ret = -1;
	struct aw_dsp_ng_desc *dsp_ng_desc = &aw_dev->dsp_ng_desc;
	...
	ret = aw_dev->ops.aw_dsp_write_bits(aw_dev, dsp_ng_desc->dsp_lp_reg,
			dsp_ng_desc->dsp_lp_mask, val, dsp_ng_desc->dsp_lp_data_type);
	if (ret < 0) {
					...
					uint32_t spin_angle)
{
	...
	struct aw_chanslot_desc *chanslot_desc = &aw_pa->chanslot_desc;
	struct aw_spin_ch *spin_slot = &aw_pa->spin_desc.spin_slot_table[spin_angle];
	...
	if (aw_pa->chansel_mode == AW_CHANSEL_TDM4) {
		ret = aw_pa->ops.aw_reg_write_bits(aw_pa, chanslot_desc->rxlslot_reg,
					chanslot_desc->rxlslot_mask, spin_slot->rx_val);
		if (ret < 0) {
			aw_dev_err(aw_pa->dev, "set slot rx failed");
			return ret;
		}
	}
...
static int aw_dev_set_params(struct aw_device *aw_dev, dev_params_t type, uint32_t *params, uint32_t len)
{
	...
	case AW_DEV_DSP_NOISE_GATE_PARAMS:
		return aw883xx_set_dsp_noise_gate_params(aw_dev, params[0]);
	case AW_DEV_DSP_LOW_POWER_PARAMS:
		return aw883xx_set_dsp_lowe_power_params(aw_dev, params[0]);
	default:
