					...
					struct device_node *np)
{
	...
	aw_dev->chansel_mode = AW_CHANSEL_TDM4;

	for (i = 0; i < AW_SPIN_MAX; i++) {
		if (spin_table_str[i] == '0') {
			aw_dev->spin_desc.spin_slot_table[i].rx_val = aw_dev->chanslot_desc.rxlslot0;
			aw_dev->spin_desc.spin_table[i].rx_val = aw_dev->chansel_desc.rx_left;
			spin_count++;
		} else if (spin_table_str[i] == '1') {
			aw_dev->spin_desc.spin_slot_table[i].rx_val = aw_dev->chanslot_desc.rxlslot1;
			aw_dev->spin_desc.spin_table[i].rx_val = aw_dev->chansel_desc.rx_left;
			spin_count++;
		} else if (spin_table_str[i] == '2') {
			aw_dev->spin_desc.spin_slot_table[i].rx_val = aw_dev->chanslot_desc.rxlslot2;
			aw_dev->spin_desc.spin_table[i].rx_val = aw_dev->chansel_desc.rx_left;
			spin_count++;
		} else if (spin_table_str[i] == '3') {
			aw_dev->spin_desc.spin_slot_table[i].rx_val = aw_dev->chanslot_desc.rxlslot3;
			aw_dev->spin_desc.spin_table[i].rx_val = aw_dev->chansel_desc.rx_left;
			spin_count++;
		} else {
			aw_dev->chansel_mode = AW_CHANSEL_I2S;
			aw_dev_err(aw_dev->dev, "no find slot, check L R");
			break;
		}
	}

	if (aw_dev->chansel_mode == AW_CHANSEL_I2S) {
		for (i = 0; i < AW_SPIN_MAX; i++) {
